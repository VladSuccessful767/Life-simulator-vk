...
function addReputation(amount) {
    ...
    // Верхнего потолка больше нет: репутация может расти 100, 150, 200 и выше.
    ...
    // Важные репутационные рубежи открывают новые этапы развития сервиса.
    const milestones = [100, 150, 200, 300, 400];
    for (const milestone of milestones) {
        if (before < milestone && state.reputation >= milestone) {
            showToast(`⭐ Репутация ${milestone}! Доступен новый этап развития сервиса.`);
            break;
        }
    }
...
function renderBase() {
    ...
    if (goalMoney) {
        goalMoney.textContent = `${money(state.money)} / 25 000 ₽`;
    }

    const goalXp = $("goalXp");
    if (goalXp) {
        goalXp.textContent = `${state.xp} / ${need}`;
    }

    // Динамическая цель опыта: после повышения уровня показываем актуальную норму.
    const goalXpEl = $("goalXp");
    if (goalXpEl) {
        const goalBox = goalXpEl.closest(".goal");
        const title = goalBox?.querySelector("b");
        const subtitle = goalBox?.querySelector("small");
        if (title) title.textContent = `Получить ${need} опыта`;
        if (subtitle) subtitle.textContent = "Повышение уровня";
    }
...
function performRepair() {
    ...
    // Всегда сохраняем конкретное название выбранной запчасти.
    // Это защищает результат ремонта от старых/неполных заказов,
    // где partName ещё не был записан.
    const selectedPart = (order.partOptions || []).find(
        part => part.id === order.selectedPartId
    );
    const installedPartName =
        repair.partName ||
        selectedPart?.name ||
        repair.name;

    repair.partName = installedPartName;
    ...
    openModal(`
        ...
        <div class="stat-row">
            <span>🔩 Установленная запчасть</span>
            <b>${installedPartName}</b>
        </div>

        <div class="stat-row">
            <span>⏱️ Норма времени</span>
            <b>${(repair.time / 60).toFixed(1)} н/ч (${repair.time} мин)</b>
        </div>
...
function openService() {
    ...
    const upgrades=[
        ...
        {level:6,name:"Большой автосервис",cost:400000,rep:100,desc:"Новый уровень сервиса: больше клиентов, расширенный склад и подготовка к найму механиков."},
        {level:7,name:"Профессиональная СТО",cost:650000,rep:150,desc:"Открываются более дорогие ремонты, увеличивается поток клиентов и число рабочих мест."},
        {level:8,name:"Сервисный центр",cost:1000000,rep:200,desc:"Продвинутая диагностика, дорогие автомобили и более прибыльные заказы."},
        {level:9,name:"Премиальный техцентр",cost:1600000,rep:300,desc:"Премиальный сегмент, сложные работы и высокий доход с каждого заказа."},
        {level:10,name:"Сеть автосервисов",cost:2500000,rep:400,desc:"Максимальный этап текущей версии: несколько постов, механики и подготовка к сети филиалов."}
    ];
...
function buyServiceUpgrade(level,cost,requiredRep){
    ...
    if(level===5)state.garage.posts=4;
    if(level===6)state.garage.posts=5;
    if(level===7)state.garage.posts=6;
    if(level===8)state.garage.posts=7;
    if(level===9)state.garage.posts=8;
    if(level>=10)state.garage.posts=10;
