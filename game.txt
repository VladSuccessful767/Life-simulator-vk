...
function openAuction() {
    ...
    const listings = state.auctionListings || [];
    openModal(`
        <h2>🔨 Аукцион</h2>
        <p style="opacity:.82">Здесь можно найти автомобиль дешевле рынка. Но состояние и скрытые неисправности нужно проверить до покупки.</p>
        <div class="auction-grid">
            ${listings.map((car, i) => `
                <div class="auction-card">
                    <div class="auction-photo-wrap">
                        ${auctionCarImage(car)}
                    </div>
                    <h3>${car.brand} ${car.model}</h3>
                    <div class="auction-meta">${car.year} г. · ${car.engine}</div>
                    <div class="auction-meta">🛣️ ${Number(car.mileage).toLocaleString("ru-RU")} км</div>
                    <div class="auction-meta">🔧 Состояние: ${car.condition}%</div>
                    <div class="auction-price">${money(car.price)}</div>
                    <div class="auction-market">Рынок: ${money(car.marketPrice)}</div>
                    <button class="action-button" onclick="inspectAuctionCar(${i})">🔍 Осмотреть</button>
                    <button class="action-button" onclick="diagnoseAuctionCar(${i})">💻 Диагностика</button>
                    <button class="action-button" onclick="buyAuctionCar(${i})">💰 Купить</button>
                </div>
            `).join("")}
        </div>
        <button class="action-button secondary-button" onclick="generateAuctionListings(true);openAuction()">🔄 Обновить лоты</button>
        <button class="modal-back-button" onclick="closeModal()">← Назад</button>
    `);
}
...
function openAuctionDiagnosis(index, extra = "") {
    ...
    openModal(`
        <h2>🔍 Диагностика: ${car.brand} ${car.model}</h2>
        ...
        <div class="auction-inspection">
            ...
            <div>
                ...
                <p>⚙️ ${car.engine}</p>
                <p>⚙️ ${car.transmission}</p>
                <p>💰 Аукцион: ${money(car.price)}</p>
            </div>
        </div>

        <p><b>Выбирай проверки.</b> Скрытые неисправности не показываются автоматически.</p>

        <div class="inspection-grid">
            <button class="inspection-button" onclick="auctionRunCheck(${index}, 'visual')">
                👀 <b>Визуальный осмотр</b>
                <small>${checks.visual ? "Проверено" : "Проверить"}</small>
            </button>

            <button class="inspection-button" onclick="auctionRunCheck(${index}, 'obd')">
                💻 <b>OBD-II</b>
                <small>${auctionCheckAvailable("obd") ? (checks.obd ? "Проверено" : "Проверить") : "Оборудование не куплено"}</small>
            </button>

            <button class="inspection-button" onclick="auctionRunCheck(${index}, 'multimeter')">
                ⚡ <b>Мультиметр</b>
                <small>${auctionCheckAvailable("multimeter") ? (checks.multimeter ? "Проверено" : "Проверить") : "Оборудование не куплено"}</small>
            </button>

            <button class="inspection-button" onclick="auctionRunCheck(${index}, 'lift')">
                🏗️ <b>Подъёмник</b>
                <small>${auctionCheckAvailable("lift") ? (checks.lift ? "Проверено" : "Проверить") : "Оборудование не куплено"}</small>
            </button>

            <button class="inspection-button" onclick="auctionRunCheck(${index}, 'smoke')">
                💨 <b>Дымогенератор</b>
                <small>${auctionCheckAvailable("smoke") ? (checks.smoke ? "Проверено" : "Проверить") : "Оборудование не куплено"}</small>
            </button>

            <button class="inspection-button" onclick="auctionRunCheck(${index}, 'pressure')">
                📏 <b>Давление</b>
                <small>${auctionCheckAvailable("pressure") ? (checks.pressure ? "Проверено" : "Проверить") : "Манометр не куплен"}</small>
            </button>

            <button class="inspection-button" onclick="auctionRunCheck(${index}, 'timing')">
                ⛓️ <b>ГРМ</b>
                <small>${auctionCheckAvailable("timing") ? (checks.timing ? "Проверено" : "Проверить") : "Нужен подъёмник и 3 уровень"}</small>
            </button>
        </div>
        ...
        <div class="diagnosis-result">
            <b>📋 Уже найдено:</b><br>
            ${revealed.length
                ? revealed.map(x => `• ${x}`).join("<br>")
                : "Пока ничего конкретного не подтверждено."}
        </div>

        <button class="action-button" onclick="buyAuctionCar(${index})">
            💰 Купить за ${money(car.price)}
        </button>
    ...
    return `
        <div class="garage-scene" id="garageScene">
            ...
            <div class="garage-actions">
                ...
                <button class="garage-action-button" onclick="openAuction()">🔨 Аукцион <span>${(state.auctionListings||[]).length}</span></button>
                <button class="garage-action-button" onclick="openMyCars()">🚗 Мои авто <span>${(state.ownedCars||[]).length}</span></button>
            </div>
