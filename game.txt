...
/* =========================================================
   ЗАЩИТА КОНТЕНТА ОТ НИЖНЕЙ НАВИГАЦИИ — v06.3
   Модальные окна получают нижний запас, чтобы фиксированное
   меню не перекрывало последние кнопки, историю и результаты.
========================================================= */
(function injectBottomNavSafeArea(){
    if(document.getElementById("bottomNavSafeAreaStyles")) return;

    const style = document.createElement("style");
    style.id = "bottomNavSafeAreaStyles";
    style.textContent = `
        @media (max-width: 700px){
            /* Нижнее меню остаётся видимым, но больше не закрывает контент модалки. */
            #modal.show{
            ...
            /* Для длинных окон прокрутка должна доходить до самого последнего элемента. */
            #modal.show #modalContent > :last-child{
