...
const problems = [
    ...
    },
    {
        ...
        repairs:{spark:{name:"Замена свечей зажигания",partName:"Комплект свечей зажигания",partCost:2800,workCost:1100,time:35},coil:{name:"Замена катушки зажигания",partCost:4200,workCost:900,time:30},injector:{name:"Диагностика и чистка форсунки",partCost:3500,workCost:2400,time:100},compression:{name:"Ремонт двигателя по причине низкой компрессии",partCost:15000,workCost:12000,time:480}}
    },
    {
        ...
        repairs:{valve_cover:{name:"Замена прокладки клапанной крышки",partCost:1800,workCost:1500,time:70},sump:{name:"Герметизация/ремонт поддона",partCost:1400,workCost:1800,time:80},seal:{name:"Замена переднего сальника коленвала",partName:"Передний сальник коленвала",partCost:1800,workCost:6500,time:240},filter:{name:"Устранение течи масляного фильтра",partCost:900,workCost:700,time:25}}
    },
    {
    ...
    },
    {
        ...
        repairs:{belt:{name:"Замена комплекта ремня ГРМ",partName:"Комплект ремня ГРМ",partCost:8500,workCost:6500,time:300},tensioner:{name:"Замена роликов и натяжителя ГРМ",partName:"Комплект роликов и натяжитель ГРМ",partCost:6500,workCost:5000,time:240},waterpump:{name:"Замена помпы вместе с приводом ГРМ",partName:"Водяная помпа + комплект привода ГРМ",partCost:7000,workCost:6500,time:300},ok:{name:"Плановая проверка без ремонта",partCost:0,workCost:700,time:30,serviceOnly:true}}
    },
    {
    ...
    {
        id:"clutch_slip",category:"transmission",transmissionType:"manual",title:"Сцепление буксует",minLevel:5,difficulty:"🟠 Сложная",
        ...
        repairs:{clutch:{name:"Замена комплекта сцепления",partName:"Комплект сцепления",partCost:12000,workCost:7000,time:360},oil:{name:"Устранение течи и замена сцепления",partCost:14500,workCost:8500,time:420},hydraulic:{name:"Ремонт гидропривода сцепления",partCost:2800,workCost:2200,time:120},normal:{name:"Проверка без ремонта",partCost:0,workCost:700,time:30,serviceOnly:true}}
    }
...
function randomWeightedList(items) {
...
function isOrderCompatible(order) {
    ...
    const def = cars.find(c => c.brand === order.car.brand && c.model === order.car.model);
    if (!def) return false;
    const engine = String(order.car.engine || "");
    const transmission = String(order.car.transmission || "");
    if (!def.engines.includes(engine) || !def.transmissions.includes(transmission)) return false;
    const diesel = /дизель/i.test(engine);
    const automatic = /АКПП|CVT|АМТ/i.test(transmission);
    if (order.problem.fuelType === "diesel" && !diesel) return false;
    if (order.problem.fuelType === "petrol" && diesel) return false;
    if (order.problem.transmissionType === "automatic" && !automatic) return false;
    if (order.problem.transmissionType === "manual" && automatic) return false;
    return true;
}
...
function generateOrder() {
    ...
    const engine = random(car.engines);
    const transmission = random(car.transmissions);
    const diesel = /дизель/i.test(engine);
    const automatic = /АКПП|CVT|АМТ/i.test(transmission);
    ...
    const eligibleProblems = problems.filter(p => {
        ...
        if (p.fuelType === "diesel" && !diesel) return false;
        if (p.fuelType === "petrol" && diesel) return false;
        if (p.transmissionType === "automatic" && !automatic) return false;
        if (p.transmissionType === "manual" && automatic) return false;
        return true;
    });
...
function openDiagnosis() {
    ...
    let html = `
        ...
        <p>
            ...
            км<br>
            🔧 Двигатель: <b>${order.car.engine}</b><br>⚙️ КПП: <b>${order.car.transmission || "—"}</b>
        </p>
...
function getRepairPartLabel(repair) {
    if (!repair) return "Запчасть";
    if (repair.partName) return repair.partName;
    const n = String(repair.name || "");
    const special = {
        "Обслуживание АКПП и замена ATF": "Трансмиссионная жидкость ATF / расходные материалы",
        "Ремонт соленоида АКПП": "Соленоид АКПП / ремкомплект",
        "Сложный ремонт АКПП": "Ремкомплект АКПП",
        "Ремонт двигателя по причине низкой компрессии": "Комплект деталей для ремонта двигателя",
        "Ремонт двигателя по причине расхода масла": "Комплект деталей для ремонта двигателя",
        "Диагностика и ремонт блока управления": "Ремкомплект / электронный блок управления",
        "Плановая проверка без ремонта": "Расходные материалы не требуются"
    };
    if (special[n]) return special[n];
    return n
        .replace(/^Замена\s+/i, "")
        .replace(/^Ремонт\/замена\s+/i, "")
        .replace(/^Ремонт\s+/i, "Ремкомплект / ")
        .replace(/^Обслуживание\s+/i, "Комплект для обслуживания ")
        .replace(/^Устранение\s+/i, "Ремкомплект для устранения ")
        .replace(/^Герметизация\/ремонт\s+/i, "Ремкомплект ")
        .replace(/^Поиск и устранение\s+/i, "Материалы для устранения ")
        .replace(/^Корректировка давления в шинах$/i, "Расходные материалы")
        .replace(/^Развал-схождение$/i, "Расходные материалы не требуются");
}

function getRepairSteps(repair) {
    if (!repair) return [];
    if (repair.steps) return repair.steps;
    const n = String(repair.name || "");
    const map = {
        "Замена переднего сальника коленвала": [
            "Подготовка и обеспечение доступа к передней части двигателя",
            "Демонтаж необходимых элементов привода",
            "Снятие старого сальника коленвала",
            "Очистка посадочного места и установка нового сальника",
            "Сборка и проверка герметичности"
        ],
        "Замена комплекта ремня ГРМ": [
            "Подготовка и установка двигателя в положение ВМТ",
            "Демонтаж кожухов и старого комплекта ГРМ",
            "Установка нового ремня и роликов",
            "Проверка совпадения меток и натяжения",
            "Проворот двигателя вручную и контрольная проверка"
        ],
        "Замена комплекта сцепления": [
            "Снятие коробки передач",
            "Демонтаж старого комплекта сцепления",
            "Проверка состояния маховика",
            "Установка нового комплекта сцепления",
            "Установка коробки и проверка работы"
        ],
        "Замена свечей накаливания": [
            "Проверка доступа и состояния свечей",
            "Демонтаж старых свечей накаливания",
            "Установка новых свечей",
            "Проверка запуска двигателя"
        ],
        "Замена свечей зажигания": [
            "Снятие катушек/наконечников",
            "Демонтаж старых свечей",
            "Установка свечей с правильным моментом затяжки",
            "Проверка работы двигателя"
        ]
    };
    return map[n] || [repair.name];
}

function getPartOptions(repair) {
    const partLabel = getRepairPartLabel(repair);
    if(repair.serviceOnly) return [{ id:"service", name:partLabel, supplier:"Услуга автосервиса", quality:"Работа / регулировка", cost:0, reliability:0.99, reputationRisk:0, icon:"📐" }];
    const base = repair.partCost;
    return [
        { id:"original", name:`${partLabel} — оригинал`, supplier:"Официальный поставщик", quality:"Оригинал", cost:base, reliability:0.98, reputationRisk:0, icon:"🟢" },
        { id:"premium", name:`${partLabel} — качественный аналог`, supplier:"Проверенный поставщик", quality:"Высокое качество", cost:Math.round(base*0.72), reliability:0.91, reputationRisk:1, icon:"🔵" },
        { id:"budget", name:`${partLabel} — бюджетный аналог`, supplier:"Эконом-сегмент", quality:"Бюджет", cost:Math.round(base*0.48), reliability:0.78, reputationRisk:3, icon:"🟡" }
    ];
}
...
function openRepairAgreement() {
    ...
    const partLabel = getRepairPartLabel(repair);
    const causeName = order.problem.causes.find(c => c.id === order.selectedCause)?.name || "Подтверждённая неисправность";
    openModal(`
        <h2>🤝 Согласование ремонта</h2>
        <p>Диагноз подтверждён. Перед выбором детали клиент должен видеть <b>конкретную работу, причину, время и стоимость труда</b>.</p>
        <div class="diagnosis-result">
            <b>📋 Заказ-наряд</b><br><br>
            👤 Клиент: ${order.client.name}<br>
            🚗 Автомобиль: ${order.car.brand} ${order.car.model}<br>
            🔍 Причина: <b>${causeName}</b><br>
            🔧 <b>Работа:</b> ${repair.name}<br>
            <div style="margin-top:10px"><b>🛠️ Операции:</b><ol style="margin:8px 0 0 20px;padding:0">${getRepairSteps(repair).map(step=>`<li style="margin:5px 0">${step}</li>`).join('')}</ol></div>
            🔩 <b>Запчасть:</b> ${partLabel}<br>
            ⏱️ <b>Норма времени:</b> ${(repair.time/60).toFixed(1)} н/ч<br>
            💰 <b>Стоимость работы:</b> ${money(repair.workCost)}
        </div>
        <h3>🔩 Варианты запчасти</h3>
...
function presentPartsToClient() {
    ...
    openModal(`
        ...
        <div class="diagnosis-result">
            🔧 <b>Работа:</b> ${repair.name}<br>
            🔩 <b>Выбрано:</b> ${selected.name}<br>
            ⏱️ <b>Норма времени:</b> ${(repair.time/60).toFixed(1)} н/ч<br>
            🔧 <b>Работа:</b> ${money(repair.workCost)}<br>
            Закупочная цена: <b>${money(selected.cost)}</b>
        </div>
