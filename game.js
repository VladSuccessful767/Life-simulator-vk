...
function getCheckTool(key) {
    const tools = {
        visual: "👀 Визуальный осмотр — без специального оборудования",
        battery: "🔌 Мультиметр / тестер аккумулятора",
        spark: "🔧 Свечной ключ / базовый инструмент",
        fuel: "📈 Манометр давления топлива",
        obd: "💻 OBD-II сканер",
        live: "💻 OBD-II сканер",
        throttle: "🔧 Базовый инструмент",
        smoke: "💨 Дымогенератор",
        lift: "🏗️ Подъёмник + базовый инструмент",
        shock: "🛞 Базовый инструмент",
        brake: "🛑 Базовый инструмент / измерительный инструмент",
        temperature: "🌡️ OBD-II / термометр",
        fan: "🔌 Мультиметр / контрольная лампа",
        radiator: "🔧 Базовый инструмент",
        coolant: "💧 Без специального оборудования",
    };
    return tools[key] || "🧰 Базовый инструмент";
}
...
function openDiagnosis() {
    ...
    let html = `
        ...
        <button class="action-button secondary-button" onclick="openTools()">🧰 Какие инструменты доступны?</button>
...
function getPartOptions(repair) {
    if (!repair || repair.partCost <= 0) {
        return [{
            id: "service",
            name: repair?.name || "Работа без замены детали",
            type: "Работа",
            partCost: 0,
            quality: "Без замены детали",
            description: "Деталь не требуется или ремонт выполняется без установки новой запчасти."
        }];
    }
    ...
    return [
        {
            id: "original",
            name: `Оригинал — ${repair.name}`,
            type: "Оригинальная деталь",
            partCost: Math.round(repair.partCost),
            quality: 1,
            description: "Максимальная совместимость и качество, но самая высокая цена."
        },
        {
            id: "premium",
            name: `Хороший аналог — ${repair.name}`,
            type: "Премиальный аналог",
            partCost: Math.round(repair.partCost * 0.72),
            quality: 0.88,
            description: "Хороший баланс цены и качества."
        },
        {
            id: "budget",
            name: `Бюджетный аналог — ${repair.name}`,
            type: "Бюджетный аналог",
            partCost: Math.round(repair.partCost * 0.48),
            quality: 0.72,
            description: "Дешевле, но клиент принимает больший компромисс по ресурсу и качеству."
        }
    ];
}
...
function openRepairAgreement() {
    ...
    openModal(`
        ...
        <div class="stat-row"><span>Автомобиль</span><b>${order.car.brand} ${order.car.model}</b></div>
        <div class="stat-row"><span>Неисправность</span><b>${repair.name}</b></div>
        <p>Предложи клиенту варианты. Ты сам задаёшь цену каждого варианта, а клиент выберет подходящий по цене и качеству.</p>

        <div class="part-option-list">
            ${options.map((o, i) => {
                const cost = o.partCost + repair.workCost;
                const markup = o.partCost > 0 ? Math.max(500, Math.round(o.partCost * 0.25 / 100) * 100) : 800;
                return `
                    <div class="part-option-card">
                        <div class="part-option-head"><b>${o.name}</b><span>Себестоимость ${money(cost)}</span></div>
                        <small>${o.description}</small>
                        <div class="stat-row"><span>🔩 Запчасть</span><b>${money(o.partCost)}</b></div>
                        <div class="stat-row"><span>🔧 Работа</span><b>${money(repair.workCost)}</b></div>
                        <label class="price-label">Цена клиенту</label>
                        <input id="repairPrice${i}" class="price-input" type="number" value="${cost + markup}" min="${cost}" step="100">
                    </div>
                `;
            }).join("")}
        </div>

        <div class="diagnosis-result">💡 Клиент увидит все предложенные варианты и сам выберет один. Слишком высокая наценка может привести к отказу от всех вариантов.</div>
        <button class="action-button" onclick="offerRepairOptions()">🤝 Показать варианты клиенту</button>
    `);
}

function offerRepairOptions() {
    const order = state.currentOrder;
    const repair = order.problem.repairs[order.selectedCause];
    const options = getPartOptions(repair);
    const offers = [];
    ...
    options.forEach((option, i) => {
        const cost = option.partCost + repair.workCost;
        const price = Number($("repairPrice" + i)?.value);
        if (!price || price < cost) return;
        const markup = (price - cost) / Math.max(1, cost);
        offers.push({ option, cost, price, markup });
    });

    if (!offers.length) {
        showToast("Проверь цены: они не могут быть ниже себестоимости");
        return;
    }

    const scored = offers.map(offer => {
        const qualityScore = (offer.option.quality || 0.8) * 0.55;
        const pricePenalty = offer.markup * order.client.priceSensitivity * 0.9;
        const reputationBonus = Math.max(0, Math.min(0.12, state.reputation / 1000));
        const score = qualityScore - pricePenalty + reputationBonus + (Math.random() * 0.08 - 0.04);
        return { ...offer, score };
    }).sort((a, b) => b.score - a.score);

    const best = scored[0];
    const baseAcceptance = 0.94 - best.markup * order.client.priceSensitivity;
    const qualityAcceptance = (best.option.quality || 0.8) * 0.12;
    const accepted = Math.random() < Math.max(0.10, Math.min(0.97, baseAcceptance + qualityAcceptance));

    order.offers = offers.map(x => ({
        name: x.option.name,
        type: x.option.type,
        partCost: x.option.partCost,
        price: x.price
    }));
    ...
    if (!accepted) {
        ...
        openModal(`
            ...
            <p>Клиент посмотрел все варианты, но не нашёл подходящий по цене и качеству.</p>
            <div class="diagnosis-result">
                💬 ${order.client.name}: «Давайте пока не будем ремонтировать».<br>
                💰 Диагностика оплачивается: <b>${money(order.diagnosisPrice)}</b>
            </div>
            <button class="action-button" onclick="openRepairAgreement()">🔁 Пересогласовать цены</button>
            <button class="action-button secondary-button" onclick="finishDiagnosisOnly()">📋 Завершить заказ</button>
        `);
        return;
    }

    order.selectedPart = best.option;
    order.repair = {
        name: best.option.name,
        partType: best.option.type,
        partQuality: best.option.quality || 1,
        partCost: best.option.partCost,
        workCost: repair.workCost,
        time: repair.time,
        cost: best.cost,
        price: best.price,
        profit: best.price - best.cost
    };
    order.agreed = true;
    save();

    openModal(`
        <h2>✅ Клиент выбрал запчасть</h2>
        <div class="diagnosis-result">
            👤 ${order.client.name} выбрал: <b>${best.option.type}</b><br>
            🔩 ${best.option.name}<br>
            💰 Цена: <b>${money(best.price)}</b>
        </div>
        <div class="stat-row"><span>Себестоимость</span><b>${money(best.cost)}</b></div>
        <div class="stat-row"><span>Твоя прибыль</span><b>${money(best.price - best.cost)}</b></div>
...
function openTools() {
    openModal(`
        ...
        <div class="diagnosis-result">
            Инструменты теперь используются прямо во время диагностики.
            Выбирай проверку, смотри её требования и следи за ресурсом оборудования.
        </div>
        <div class="stat-row"><span>🧰 Ресурс оборудования</span><b>${state.equipment}%</b></div>
        <div class="knowledge-block">
            <h3>Что уже доступно</h3>
            <p>👀 Визуальный осмотр — без оборудования</p>
            <p>🔧 Базовый набор — простые механические проверки</p>
            <p>💻 OBD-II — ошибки и Live Data</p>
            <p>🔌 Мультиметр — электрические проверки</p>
            <p>📈 Манометр — давление топлива и другие измерения</p>
            <p>💨 Дымогенератор — поиск подсоса воздуха</p>
            <p>🏗️ Подъёмник — доступ к ходовой части</p>
        </div>
        <p class="hint">Позже инструменты можно будет покупать и улучшать. Пока они доступны на базовом уровне, а ресурс расходуется за проверки.</p>
    `);
}
