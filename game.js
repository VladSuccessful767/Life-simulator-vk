...
state.totalEarnings = Number.isFinite(Number(state.totalEarnings)) ? Number(state.totalEarnings) : Math.max(0, (state.money || 15000) - 15000);
state.completedOrders = Number.isFinite(Number(state.completedOrders)) ? Number(state.completedOrders) : (Array.isArray(state.history) ? state.history.length : 0);
state.goalsClaimed = state.goalsClaimed && typeof state.goalsClaimed === "object" ? state.goalsClaimed : {};
...
function renderBase() {
    ...
    if (goalMoney) {
        const done = state.totalEarnings >= 25000;
        goalMoney.innerHTML = done ? `✅ Выполнено` : `${money(state.totalEarnings)} / 25 000 ₽`;
        goalMoney.classList.toggle("goal-done", done);
    }

    const goalXp = $("goalXp");
    if (goalXp) {
        const done = state.xp >= 100;
        goalXp.innerHTML = done ? `✅ Выполнено` : `${Math.min(state.xp,100)} / 100`;
        goalXp.classList.toggle("goal-done", done);
    }

    const goalOrders = $("goalOrders");
    if (goalOrders) {
        const done = state.completedOrders >= 3;
        goalOrders.innerHTML = done ? `✅ Выполнено` : `${Math.min(state.completedOrders,3)} / 3`;
        goalOrders.classList.toggle("goal-done", done);
    }
...
function startDiagnosis() {
    ...
    // Новый заказ нельзя взять при критическом износе оборудования.
    if (!state.currentOrder && Number(state.equipment) < 10) {
        openEquipmentBlockedModal();
        return;
    }
...
function performCheck(key) {
    ...
    // Мягкий износ: простые проверки ≈ 1%, более серьёзные ≈ 2%.
    // Сама стоимость оборудования в данных проверки остаётся отдельной
    // характеристикой и не привязана напрямую к процентам износа.
    const equipmentWear = check.equipment <= 0 ? 0 : (check.equipment <= 2 ? 1 : 2);
    useEquipment(equipmentWear);
...
function getRepairPartName(repair) {
    if (!repair || repair.serviceOnly) return repair?.name || "Услуга";
    const n = String(repair.name || "");
    const map = [
        [/радиатор/i, "Радиатор"],
        [/патрубк/i, "Патрубок системы охлаждения"],
        [/водяной помп|помп/i, "Водяная помпа"],
        [/термостат/i, "Термостат"],
        [/генератор/i, "Генератор"],
        [/приводного ремня/i, "Приводной ремень"],
        [/аккумулятор/i, "Аккумулятор"],
        [/свечей зажигания/i, "Свечи зажигания"],
        [/катушк/i, "Катушка зажигания"],
        [/форсунк/i, "Форсунка"],
        [/тормозных колодок/i, "Тормозные колодки"],
        [/тормозных дисков/i, "Тормозные диски"],
        [/суппорт/i, "Тормозной суппорт"],
        [/ступичного подшипника/i, "Ступичный подшипник"],
        [/шины|шину/i, "Шина"],
        [/колёсного диска/i, "Колёсный диск"],
        [/сальника/i, "Сальник коленчатого вала"],
        [/прокладки клапанной крышки/i, "Прокладка клапанной крышки"],
        [/поддона/i, "Прокладка/герметик поддона"],
        [/масляного фильтра/i, "Масляный фильтр"],
        [/компрессора кондиционера/i, "Компрессор кондиционера"],
        [/салонного фильтра/i, "Салонный фильтр"],
        [/турбокомпрессора/i, "Турбокомпрессор"],
        [/датчика давления наддува/i, "Датчик давления наддува"],
        [/комплекта ремня ГРМ/i, "Комплект ГРМ"],
        [/роликов и натяжителя ГРМ/i, "Ролики и натяжитель ГРМ"],
        [/сцепления/i, "Комплект сцепления"],
        [/рулевой рейки/i, "Рулевая рейка"],
        [/рулевого наконечника|тяги/i, "Рулевой наконечник/тяга"],
        [/шаровой опоры/i, "Шаровая опора"],
        [/дизельной форсунки/i, "Дизельная форсунка"],
        [/свечей накаливания/i, "Свечи накаливания"]
    ];
    for (const [re, name] of map) if (re.test(n)) return name;
    return "Запчасть";
}
...
function getPartOptions(repair) {
    ...
    const partName = repair.partName || getRepairPartName(repair);
    return [
        { id:"original", name:`${partName} — оригинал`, supplier:"Официальный поставщик", quality:"Оригинал", cost:base, reliability:0.98, reputationRisk:0, icon:"🟢" },
        { id:"premium", name:`${partName} — качественный аналог`, supplier:"Проверенный поставщик", quality:"Высокое качество", cost:Math.round(base*0.72), reliability:0.91, reputationRisk:1, icon:"🔵" },
        { id:"budget", name:`${partName} — бюджетный аналог`, supplier:"Эконом-сегмент", quality:"Бюджет", cost:Math.round(base*0.48), reliability:0.78, reputationRisk:3, icon:"🟡" }
    ];
}
...
function checkGoals() {
    const goals = [
        { id:"money25", condition:state.totalEarnings >= 25000, reward:3000, text:"🎯 Цель выполнена: заработано 25 000 ₽" },
        { id:"xp100", condition:state.xp >= 100, reward:2000, text:"🎯 Цель выполнена: получено 100 опыта" },
        { id:"orders3", condition:state.completedOrders >= 3, reward:4000, text:"🎯 Цель выполнена: выполнено 3 заказа" }
    ];
    const fresh = [];
    for (const goal of goals) {
        if (goal.condition && !state.goalsClaimed[goal.id]) {
            state.goalsClaimed[goal.id] = true;
            state.money += goal.reward;
            fresh.push(`${goal.text}. Награда +${money(goal.reward)}`);
        }
    }
    return fresh;
}
...
function performRepair() {
    ...
    // Износ ремонта зависит от его сложности: 1–3%.
    const repairWear = repair.time <= 60 ? 1 : (repair.time <= 180 ? 2 : 3);
    useEquipment(repairWear);


    state.money += repair.profit;
    state.money += order.diagnosisPrice;
    state.totalEarnings += Math.max(0, repair.profit) + Math.max(0, order.diagnosisPrice);
    ...
    openModal(`
        ...
        <div class="stat-row">
            <span>🔩 Запчасть</span>
            <b>${order.repair?.partName || repair.partName || repair.name}</b>
        </div>
        ...
        ${freshGoals.length ? `<div class="diagnosis-result" style="margin-top:12px">${freshGoals.map(x=>`🏆 ${x}`).join('<br>')}</div>` : ''}
...
function finishDiagnosisOnly() {
    ...
    if (
        ...
        state.money += order.diagnosisPrice;
        state.totalEarnings += Math.max(0, order.diagnosisPrice);
        state.completedOrders += 1;
        checkGoals();
...
function newOrder() {
    ...
    // При ресурсе ниже 10% новый заказ недоступен до обслуживания оборудования.
    if (Number(state.equipment) < 10) {
        openEquipmentBlockedModal();
        return;
    }
    ...
    /*
       Оборудование больше НЕ восстанавливается автоматически.
       Восстановление выполняется через обслуживание или замену.
    */

    state.equipment =
        Math.max(0, Math.min(100, Number(state.equipment) || 0));
...
function buyServiceUpgrade(level,cost,requiredRep){
    ...
    state.equipment=Math.min(100,Math.max(0,state.equipment+25));
    addXP(40);
    save();render();
...
function openEquipmentBlockedModal() {
    const eq = Math.max(0, Math.min(100, Number(state.equipment) || 0));
    openModal(`
        <h2>🚫 Заказ недоступен</h2>
        <div class="diagnosis-result">
            🧰 Оборудование изношено до <b>${eq}%</b>.<br><br>
            При ресурсе ниже <b>10%</b> сервис не может принимать новый автомобиль:
            диагностическое оборудование требует обслуживания.
        </div>
        <button class="action-button" onclick="openEquipmentMaintenanceMenu()">🧰 Обслужить оборудование</button>
        <button class="action-button secondary-button" onclick="closeModal()">← Закрыть</button>
    `);
}

function openEquipmentMaintenanceMenu() {
    const eq = Math.max(0, Math.min(100, Number(state.equipment) || 0));
    const canBasic = state.money >= 500 && eq < 100;
    const canFull = state.money >= 1500 && eq < 100;
    const canNew = state.money >= 50000 && eq < 100;

    openModal(`
        <h2>🧰 Обслуживание оборудования</h2>
        <div class="stat-row"><span>Текущий ресурс</span><b>${eq}%</b></div>
        <div class="diagnosis-result">
            Выбери, как восстановить оборудование. Покупка нового комплекта сразу возвращает ресурс к <b>100%</b>.
        </div>

        <div class="upgrade-card">
            <h3>🔧 Обычное обслуживание</h3>
            <p>Базовое обслуживание и замена расходников.</p>
            <div class="stat-row"><span>Стоимость</span><b>500 ₽</b></div>
            <div class="stat-row"><span>Восстановление</span><b>+5%</b></div>
            <button class="action-button" ${(!canBasic)?'disabled':''} onclick="maintainEquipment(500,5)">🔧 Обслужить</button>
        </div>

        <div class="upgrade-card">
            <h3>🛠️ Полное ТО</h3>
            <p>Полная проверка, настройка и обслуживание оборудования.</p>
            <div class="stat-row"><span>Стоимость</span><b>1 500 ₽</b></div>
            <div class="stat-row"><span>Восстановление</span><b>+15%</b></div>
            <button class="action-button" ${(!canFull)?'disabled':''} onclick="maintainEquipment(1500,15)">🛠️ Полное ТО</button>
        </div>

        <div class="upgrade-card">
            <h3>🆕 Купить новое оборудование</h3>
            <p>Полная замена комплекта диагностического оборудования.</p>
            <div class="stat-row"><span>Стоимость</span><b>50 000 ₽</b></div>
            <div class="stat-row"><span>Ресурс после покупки</span><b>100%</b></div>
            <button class="action-button" ${(!canNew)?'disabled':''} onclick="buyNewEquipment()">🛒 Купить новое оборудование</button>
        </div>

        ${eq >= 100 ? '<div class="diagnosis-result">✅ Оборудование уже полностью исправно.</div>' : ''}
        <button class="modal-back-button" onclick="closeModal()">← Закрыть</button>
    `);
}

function buyNewEquipment() {
    const cost = 50000;
    if (state.equipment >= 100) {
        showToast('🧰 Оборудование уже на 100%');
        openEquipmentMaintenanceMenu();
        return;
    }
    if (state.money < cost) {
        showToast(`💰 Нужно ${money(cost)} для покупки нового оборудования`);
        openEquipmentMaintenanceMenu();
        return;
    }
    state.money -= cost;
    ...
    save();
    render();
    showToast('🆕 Новое оборудование установлено — ресурс 100%');
    openEquipmentMaintenanceMenu();
}
...
function maintainEquipment(cost, restore) {
    const eq = Math.max(0, Math.min(100, Number(state.equipment) || 100));
    ...
    if (state.money < cost) {
        showToast(`💰 Нужно ${money(cost)} для обслуживания`);
        openGarageTools();
        return;
    }

    state.money -= cost;
    state.equipment = Math.min(100, eq + restore);
    addTime(cost === 1500 ? 30 : 10);
    save();
    render();
    showToast(`🧰 Оборудование восстановлено до ${state.equipment}%`);
    openGarageTools();
}
...
function openGarageParts() {
    ...
    const rows = items.length ? items.map((item, i) => `
        <div class="diagnosis-result" style="margin-top:10px">
            ...
            🚗 ${item.car}<br>
            Состояние: <b>${item.condition}%</b><br>
            <span style="opacity:.8">🔒 Восстановление будет доступно после покупки специального оборудования.</span>
        </div>
    `).join('') : `
        <div class="diagnosis-result">Склад пока пуст. Старые детали, которые клиенты оставят после ремонта, можно будет хранить здесь.</div>
    `;
    ...
    openModal(`
        ...
        ${rows}
        <p>🏭 Позже появится оборудование для восстановления отдельных деталей и их дальнейшей реализации.</p>
        <button class="modal-back-button" onclick="closeModal()">← Назад</button>
    `);
}
