...
const defaultState = {
    ...
    tools: {
        ...
        manometer: false, smoke: false, lift: false, alignment: false
    }
};
...
function offerSelectedPart() {
    ...
    if(!accepted){
        ...
        openModal(`<h2>❌ Клиент отказался от цены</h2><div class="diagnosis-result">Цена <b>${money(totalPrice)}</b> показалась клиенту высокой.<br><br>Попробуй уменьшить наценку. Запчасть менять уже не нужно — клиент свой вариант выбрал.</div><button class="action-button" onclick="openPartMarkup()">💰 Изменить наценку</button><button class="action-button secondary-button" onclick="finishDiagnosisOnly()">📋 Завершить заказ</button>`);
        return;
    }
    order.agreed=true;save();
    openModal(`<h2>✅ Клиент согласовал цену</h2><div class="diagnosis-result"><b>${part.name}</b><br>Закупка: ${money(part.cost)}<br>Цена детали: ${money(partPrice)}<br>Работа: ${money(repair.workCost)}<hr><b>Итого: ${money(totalPrice)}</b><br>Прибыль с заказа: <b>+${money(totalPrice-part.cost)}</b></div><button class="action-button" onclick="performRepair()">🔧 Выполнить ремонт</button>`);
}
...
function performRepair() {
    ...
    addReputation(8 - (order.repair.reputationRisk || 0));
...
function openService() {
    ...
    const upgrades=[
        {level:2,name:"Второй пост",cost:25000,desc:"Больше клиентов и +1 пост."},
        {level:3,name:"Подъёмник и большой гараж",cost:60000,desc:"Сложные работы, больше вариантов диагностики."},
        {level:4,name:"Диагностический пост",cost:120000,desc:"Лучший ресурс оборудования и доступ к продвинутым заказам."},
        {level:5,name:"Небольшая СТО",cost:250000,desc:"Подготовка к найму механиков и нескольким заказам."}
    ];
    ...
    openModal(`
        ...
        ${next?`<div class="upgrade-card"><h3>Следующий этап: ${next.name}</h3><p>${next.desc}</p><div class="stat-row"><span>Стоимость</span><b>${money(next.cost)}</b></div><button class="action-button" ${state.money<next.cost?'disabled':''} onclick="buyServiceUpgrade(${next.level},${next.cost})">🏗️ Улучшить сервис</button></div>`:`<div class="knowledge-practice"><h3>🏆 Максимальный доступ текущей версии</h3><p>Дальше можно расширять СТО, нанимать механиков, открывать вакансии и сеть сервисов.</p></div>`}
        <div class="diagnosis-result">💡 Сейчас основной заработок — диагностика и ремонт. Чем выше уровень сервиса, тем больше возможностей появится в следующих обновлениях.</div>
    `);
}

function buyServiceUpgrade(level,cost){
    if(state.garage.level+1!==level || state.money<cost){showToast("Недостаточно денег для улучшения");return;}
    state.money-=cost;
