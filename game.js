...
const trainingTests = {
    timing: {
        ...
        description: "Перед серьёзной работой с ГРМ мастер должен подтвердить знания. Тест проходит без подсказок. Одна ошибка — допуск не получен.",
        questions: [
            {
                q: "Какова основная задача ГРМ?",
                a: ["Синхронизировать движение коленчатого и распределительных валов", "Охлаждать масло", "Заряжать аккумулятор", "Создавать давление топлива"],
                correct: 0
            },
            {
                q: "Что необходимо проверить после установки ремня ГРМ?",
                a: ["Только уровень масла", "Совпадение меток и правильное натяжение", "Только давление в шинах", "Только заряд аккумулятора"],
                correct: 1
            },
            {
                q: "Что может произойти при серьёзном нарушении фаз ГРМ?",
                a: ["Только разрядится аккумулятор", "Двигатель может не запуститься", "Увеличится давление в шинах", "Перестанет работать кондиционер"],
                correct: 1
            },
            {
                q: "Что входит в проверку ремня ГРМ?",
                a: ["Только цвет ремня", "Трещины, зубья, натяжение, ролики и совпадение меток", "Только уровень топлива", "Только состояние аккумулятора"],
                correct: 1
            },
            {
                q: "Какой подход правильнее при работе с ГРМ?",
                a: ["Работать без фиксаторов и проверить всё после запуска", "Соблюдать последовательность, использовать нужный инструмент и проверить метки после установки", "Менять только ремень независимо от состояния роликов", "Сразу запускать двигатель после снятия старого ремня"],
                correct: 1
            }
        ]
    }
...
function openTrainingTests() {
    const timingPassed = !!state.training?.timing;
    openModal(`
        <div class="knowledge-header">
            <div>
                <h2>🎓 Тесты и допуски</h2>
                <p>Знания открывают доступ к более сложным работам. Сейчас доступен первый допуск — работа с ГРМ.</p>
            </div>
        </div>
        <div class="knowledge-block">
            <h3>⛓️ ГРМ</h3>
            <p>${timingPassed ? "✅ Допуск уже получен. Ты можешь переходить к практической работе, когда она будет открыта." : "🔒 Допуск не получен. Сначала изучи темы про ГРМ в базе знаний, затем пройди тест без ошибок."}</p>
        </div>
        <button class="action-button ${timingPassed ? "secondary-button" : "knowledge-study-button"}" onclick="startTrainingTest('timing')">
            ${timingPassed ? "🔁 Пройти тест ещё раз" : "📝 Пройти тест: допуск к ГРМ"}
        </button>
    ...
    window.__answerTraining = function(index) {
        ...
        if (mistakes.length === 0) {
            ...
            openModal(`
                ...
                <div class="knowledge-practice">
                    <h3>⛓️ Работы с ГРМ</h3>
                    <p>Тест пройден без ошибок. В дальнейшем этот допуск будет открывать практические задания по ГРМ.</p>
                </div>
            ...
            openModal(`
                ...
                <div class="diagnosis-result">
                    Ошибок: <b>${mistakes.length}</b>. Для допуска к работе с ГРМ тест нужно пройти без единой ошибки.
                </div>
                <div class="knowledge-block">
                    <h3>Что повторить</h3>
