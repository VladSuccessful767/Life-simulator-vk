...
const defaultState = {
    ...
    repairEquipment: {
        partsCleaner: { owned: false, resource: 100 },
        hydraulicPress: { owned: false, resource: 100 },
        repairBench: { owned: false, resource: 100 },
...
state.repairEquipment = Object.assign({}, defaultState.repairEquipment, state.repairEquipment || {});
for (const key of Object.keys(defaultState.repairEquipment)) {
    state.repairEquipment[key] = Object.assign(
        {},
        defaultState.repairEquipment[key],
        state.repairEquipment[key] || {}
    );
    state.repairEquipment[key].owned = !!state.repairEquipment[key].owned;
    state.repairEquipment[key].resource = Math.max(0, Math.min(100, Number(state.repairEquipment[key].resource ?? 100)));
}
...
function storeOldPart() {
    ...
    // В склад попадает именно снятая деталь, а не выбранный клиентом новый аналог.
    const componentName = repair.name || repair.partName || "Снятая запчасть";
    const item = {
        ...
        baseValue: Math.max(500, Number(repair.partCost || repair.partPrice || 0)),
        sourceRepair: repair.name || "",
        restoreLevel: 0
    };
    ...
    openModal(`
        <h2>📦 Старая деталь на складе</h2>
        <div class="diagnosis-result">
            <b>${componentName}</b> помещена на склад.<br><br>
            Состояние: <b>${item.condition}%</b><br>
            🚗 ${item.car}<br><br>
            🔧 Теперь деталь можно продать как есть или восстановить, если куплено подходящее оборудование.
        </div>
        <button class="action-button" onclick="newOrder()">🚗 Следующий клиент</button>
    `);
}
...
const repairEquipmentCatalog = [
    {
        id: "partsCleaner",
        icon: "🧼",
        name: "Моечная установка",
        cost: 25000,
        desc: "Очистка снятых деталей перед восстановлением.",
        restore: 10,
        wear: 3,
        requires: []
    },
    {
        ...
        cost: 50000,
        desc: "Разборка и восстановление деталей с запрессованными элементами.",
        restore: 18,
        wear: 5,
        requires: ["partsCleaner"]
    },
    {
        id: "repairBench",
        icon: "🔧",
        name: "Стенд восстановления агрегатов",
        cost: 75000,
        desc: "Позволяет полноценно восстанавливать пригодные агрегаты.",
        restore: 30,
        wear: 7,
        requires: ["partsCleaner"]
    },
    {
        id: "testBench",
        icon: "🧪",
        name: "Стенд проверки агрегатов",
        cost: 90000,
        desc: "Финальная проверка восстановленной детали перед продажей.",
        restore: 8,
        wear: 4,
        requires: ["repairBench"]
    }
];

function getRepairEquipment(id) {
    return repairEquipmentCatalog.find(x => x.id === id);
}

function getRepairEquipmentRequirement(item) {
    const name = `${item.name || ""} ${item.sourceRepair || ""}`.toLowerCase();

    // Простые детали, которые в этой версии игры не имеет смысла восстанавливать.
    if (/колодк|свеч|аккумулятор|патрубок|прокладк|ремн(я|ь)|ш(ин|инy)|диск.*колес|тормозн.*диск/.test(name)) {
        return null;
    }

    if (/генератор|стартер|компрессор|рейк|рейк[аи]|суппорт|ступич|насос|помп/.test(name)) {
        return { equipment: "repairBench", label: "Стенд восстановления агрегатов" };
    }

    if (/сцеплен|маховик|тяг|наконечник|шаровой|подшипник/.test(name)) {
        return { equipment: "hydraulicPress", label: "Гидравлический пресс" };
    }

    return { equipment: "repairBench", label: "Стенд восстановления агрегатов" };
}

function repairEquipmentOwned(id) {
    return !!state.repairEquipment?.[id]?.owned;
}

function openRepairEquipment() {
    const cards = repairEquipmentCatalog.map(eq => {
        const data = state.repairEquipment[eq.id];
        const owned = !!data?.owned;
        const requirementsOk = eq.requires.every(id => repairEquipmentOwned(id));
        const canBuy = !owned && requirementsOk && state.money >= eq.cost;
        const reqText = eq.requires.length
            ? `<small>Требуется: ${eq.requires.map(id => getRepairEquipment(id)?.name).join(", ")}</small>`
            : `<small>Базовое оборудование</small>`;

        return `
            <div class="diagnosis-result" style="margin-top:10px">
                <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start">
                    <div><b>${eq.icon} ${eq.name}</b><br><span style="opacity:.8">${eq.desc}</span></div>
                    <b>${money(eq.cost)}</b>
                </div>
                <div style="margin-top:8px">
                    ${owned
                        ? `🟢 Куплено · Ресурс: <b>${Math.round(data.resource)}%</b>`
                        : `🔴 Не куплено`}
                </div>
                ${reqText}
                ${owned
                    ? `<button class="action-button" onclick="serviceRepairEquipment('${eq.id}')">🛠️ Обслужить</button>`
                    : `<button class="action-button" ${canBuy ? "" : "disabled"} onclick="buyRepairEquipment('${eq.id}')">💰 Купить за ${money(eq.cost)}</button>`}
            </div>`;
    }).join("");

    openModal(`
        <h2>🛠️ Оборудование для восстановления</h2>
        <div class="diagnosis-result">Каждый станок покупается отдельно и имеет собственный ресурс. Более сложное оборудование открывает новые возможности восстановления.</div>
        ${cards}
        <button class="modal-back-button" onclick="openGarageParts()">← Назад на склад</button>
    `);
}

function buyRepairEquipment(id) {
    const eq = getRepairEquipment(id);
    if (!eq) return;
    const data = state.repairEquipment[id];
    if (data?.owned) { showToast("Оборудование уже куплено"); return; }
    if (state.money < eq.cost) { showToast("💸 Не хватает денег"); return; }
    if (!eq.requires.every(req => repairEquipmentOwned(req))) {
        showToast("🔒 Сначала купи необходимое оборудование");
        return;
    }
    state.money -= eq.cost;
    state.repairEquipment[id] = { owned: true, resource: 100 };
    save();
    render();
    openRepairEquipment();
}

function serviceRepairEquipment(id) {
    const eq = getRepairEquipment(id);
    const data = state.repairEquipment?.[id];
    if (!eq || !data?.owned) return;

    const cost = Math.max(1500, Math.round(eq.cost * 0.08));
    if (data.resource >= 95) {
        showToast("🟢 Оборудование почти новое");
        return;
    }
    if (state.money < cost) {
        showToast("💸 Не хватает денег на обслуживание");
        return;
    }
    state.money -= cost;
    data.resource = 100;
...
function getAsIsSalePrice(item) {
    const base = Math.max(500, Number(item.baseValue || 2000));
    const condition = Math.max(0, Math.min(100, Number(item.condition || 0)));
    return Math.max(300, Math.round(base * 0.35 * (condition / 100)));
}

function getRestoredSalePrice(item) {
    const base = Math.max(500, Number(item.baseValue || 2000));
    return Math.max(500, Math.round(base * 0.82));
}

function sellWarehousePart(index) {
    const item = state.partsWarehouse?.[index];
    if (!item) return;
    const price = item.status === "restored" ? getRestoredSalePrice(item) : getAsIsSalePrice(item);
    state.money += price;
    state.partsWarehouse.splice(index, 1);
    save();
    render();
    showToast(`💰 Деталь продана за ${money(price)}`);
    openGarageParts();
}
...
function restoreWarehousePart(index) {
    ...
    if (item.status === "restored") {
        showToast("✅ Деталь уже восстановлена");
        return;
    }

    const requirement = getRepairEquipmentRequirement(item);
    if (!requirement) {
        openModal(`
            <h2>🚫 Восстановление недоступно</h2>
            <div class="diagnosis-result">Эту деталь в текущей версии игры выгоднее заменить новой. Её можно продать как есть.</div>
            <button class="action-button" onclick="openGarageParts()">← Вернуться на склад</button>
        `);
        return;
    }

    const eq = getRepairEquipment(requirement.equipment);
    const data = state.repairEquipment[requirement.equipment];
    const cleaner = state.repairEquipment.partsCleaner;

    if (!data?.owned) {
        openModal(`
            <h2>🔒 Нет оборудования</h2>
            <div class="diagnosis-result">Для восстановления детали <b>${item.name}</b> нужен: <b>${requirement.label}</b>.</div>
            <button class="action-button" onclick="openRepairEquipment()">🛠️ Купить оборудование</button>
            <button class="modal-back-button" onclick="openGarageParts()">← Назад</button>
        `);
        return;
    }

    if (data.resource < eq.wear) {
        openModal(`
            <h2>⚠️ Оборудование изношено</h2>
            <div class="diagnosis-result">Ресурс <b>${eq.name}</b> — ${Math.round(data.resource)}%. Обслужи оборудование перед восстановлением.</div>
            <button class="action-button" onclick="serviceRepairEquipment('${eq.id}')">🛠️ Обслужить</button>
            <button class="modal-back-button" onclick="openGarageParts()">← Назад</button>
        `);
        return;
    ...
    if (cleaner?.owned && cleaner.resource >= 3) {
        cleaner.resource = Math.max(0, cleaner.resource - 3);
    }

    data.resource = Math.max(0, data.resource - eq.wear);
    item.condition = Math.min(100, Number(item.condition || 0) + eq.restore);
    item.status = "restored";
    item.restoreLevel = (item.restoreLevel || 0) + 1;
    item.restoredDay = state.day;

    // Финальный стенд повышает ценность восстановленной детали.
    if (repairEquipmentOwned("testBench")) {
        item.tested = true;
        item.saleQuality = "Проверена на стенде";
    } else {
        item.tested = false;
        item.saleQuality = "Восстановлена";
    }

    save();
    render();
    openModal(`
        <h2>🔧 Деталь восстановлена</h2>
        <div class="diagnosis-result">
            <b>${item.name}</b><br><br>
            Состояние: <b>${item.condition}%</b><br>
            ${item.saleQuality}<br>
            Оборудование: ${eq.name} · ресурс ${Math.round(data.resource)}%<br>
            💰 Цена продажи: <b>${money(getRestoredSalePrice(item))}</b>
        </div>
        <button class="action-button" onclick="openGarageParts()">📦 Вернуться на склад</button>
    `);
}
...
function openGarageParts() {
    ...
    const rows = items.length ? items.map((item, i) => {
        ...
        const asIs = getAsIsSalePrice(item);
        const restoredPrice = getRestoredSalePrice(item);
        return `
            <div class="diagnosis-result" style="margin-top:10px">
                <b>${i + 1}. ${item.name}</b><br>
                🚗 ${item.car}<br>
                Состояние: <b>${item.condition}%</b> · ${restored ? "🟢 Восстановлена" : "🟡 Б/у"}<br>
                ${item.saleQuality ? `🧪 ${item.saleQuality}<br>` : ""}
                💰 Продажа как есть: <b>${money(asIs)}</b>${restored ? ` · после восстановления: <b>${money(restoredPrice)}</b>` : ""}
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px">
                    ${restored
                        ? `<button class="action-button" onclick="sellWarehousePart(${i})">💰 Продать</button>`
                        : `<button class="action-button" onclick="restoreWarehousePart(${i})">🔧 Восстановить</button>`}
                    <button class="action-button secondary-button" onclick="sellWarehousePart(${i})">💰 Продать как есть</button>
                </div>
                ${!restored && requirement ? `<small style="display:block;margin-top:8px;opacity:.75">Для восстановления: ${requirement.label}</small>` : !restored ? `<small style="display:block;margin-top:8px;opacity:.75">Восстановление этой детали не предусмотрено — можно продать.</small>` : ""}
            </div>`;
    }).join("") : `
        <div class="diagnosis-result">Склад пока пуст. Старые детали, которые клиенты оставляют после ремонта, появляются здесь.</div>
    `;
    ...
    openModal(`
        ...
        ${rows}
        <button class="action-button" onclick="openRepairEquipment()">🛠️ Оборудование для восстановления</button>
        <button class="modal-back-button" onclick="closeModal()">← Закрыть склад</button>
    `);
}
