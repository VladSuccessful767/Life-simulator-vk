...
/* =========================================================
   АНИМАЦИИ «ПУТЬ МАСТЕРА» — v31
   Только CSS + небольшой JS-триггер, без внешних библиотек.
========================================================= */

(function installGarageAnimations(){
    if (document.getElementById("pm-garage-animation-style")) return;

    const style = document.createElement("style");
    style.id = "pm-garage-animation-style";
    style.textContent = `
        /* Жизнь автомобиля в гараже */
        .garage-car.visual-car {
            transform-origin: 50% 78%;
            will-change: transform, opacity;
            animation: pmCarIdle 4.2s ease-in-out infinite;
        }
        ...
        /* Нажали на автомобиль — начинается осмотр */
        .garage-car.visual-car.pm-car-inspect {
            animation: pmCarInspect 900ms cubic-bezier(.22,.8,.28,1) both;
        }

        @keyframes pmCarInspect {
            0%   { transform: translateY(0) scale(1); }
            28%  { transform: translateY(-10px) scale(1.018); }
            55%  { transform: translateY(-5px) scale(1.01); }
            100% { transform: translateY(0) scale(1); }
        }

        /* Фары оживают во время осмотра */
        .garage-car.visual-car.pm-car-inspect .left-headlight,
        .garage-car.visual-car.pm-car-inspect .right-headlight {
            animation: pmHeadlights 900ms ease-in-out both;
        }

        @keyframes pmHeadlights {
            0%, 100% { filter: brightness(1); opacity: .9; }
            35% { filter: brightness(1.8); opacity: 1; }
            55% { filter: brightness(1.35); opacity: 1; }
        }

        /* Тень двигается вместе с машиной */
        .garage-car.visual-car.pm-car-inspect .car-shadow {
            animation: pmCarShadow 900ms ease-in-out both;
        }

        @keyframes pmCarShadow {
            0%, 100% { transform: scaleX(1); opacity: .7; }
            28% { transform: scaleX(.88); opacity: .45; }
            55% { transform: scaleX(.94); opacity: .55; }
        }

        /* Маленький инструмент появляется на время осмотра */
        .garage-car.visual-car.pm-car-inspect::after {
            content: "🔧";
            position: absolute;
            left: 50%;
            top: -24px;
            transform: translateX(-50%) scale(.5) rotate(-25deg);
            font-size: 24px;
            opacity: 0;
            pointer-events: none;
            animation: pmTool 900ms ease-out both;
        }

        @keyframes pmTool {
            0% { opacity: 0; transform: translate(-50%, 12px) scale(.5) rotate(-25deg); }
            30% { opacity: 1; transform: translate(-50%, 0) scale(1.05) rotate(8deg); }
            70% { opacity: 1; transform: translate(-50%, -4px) scale(1) rotate(-4deg); }
            100% { opacity: 0; transform: translate(-50%, -16px) scale(.75) rotate(18deg); }
        }

        /* Подсказка слегка пульсирует, пока есть машина клиента */
        .garage-hint {
            animation: pmHint 2.8s ease-in-out infinite;
        }
        ...
        /* Нажатие кнопок — короткая физическая реакция */
        ...
        @media (prefers-reduced-motion: reduce) {
            .garage-car.visual-car,
            .garage-hint { animation: none !important; }
            .garage-car.visual-car.pm-car-inspect .left-headlight,
            .garage-car.visual-car.pm-car-inspect .right-headlight,
            .garage-car.visual-car.pm-car-inspect .car-shadow,
            .garage-car.visual-car.pm-car-inspect::after { animation: none !important; }
            .garage-action-button,
            .action-button,
            .inspection-button,
            .diagnosis-button { transition: none !important; }
        }
    `;
...
function inspectCar() {
    ...
    // Запускаем короткую анимацию осмотра автомобиля.
    const visualCar = $("visualCar");
    if (visualCar) {
        visualCar.classList.remove("pm-car-inspect");
        void visualCar.offsetWidth;
        visualCar.classList.add("pm-car-inspect");
        setTimeout(() => visualCar.classList.remove("pm-car-inspect"), 950);
    }
