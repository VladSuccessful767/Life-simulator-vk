...
function openModal(html) {
    ...
    // Каждый новый экран модального окна открываем с самого верха.
    // Скролл теперь принадлежит самому modal-window, а не странице позади.
    ...
    if (modalWindow) {
        ...
        modalWindow.style.webkitOverflowScrolling = "touch";
        modalWindow.style.overflowY = "auto";
    }

    // На мобильном браузере принудительно возвращаем фокус прокрутки
    // в модальное окно после его появления.
...
function renderBase() {
    ...
    const carMiniPhoto = document.getElementById("carMiniPhoto");
    ...
    if (order) {
        ...
        if (carMiniPhoto) {
            carMiniPhoto.style.backgroundImage = `url("${carPhotoPath(order.car)}")`;
            carMiniPhoto.title = `${order.car.brand} ${order.car.model}`;
        }
        ...
        if (carMiniPhoto) {
            carMiniPhoto.style.backgroundImage = `url("${carPhotoPath({brand:'Hyundai',model:'Solaris'})}")`;
        }
    }
...
function performCheck(key) {
    ...
    if(check.toolId && !hasTool(check.toolId)) {
        ...
        if(tool && state.level < tool.level) {
            showToast(`🔒 ${tool.name} откроется на уровне мастерства ${tool.level}`);
        } else if(tool) {
            showToast(`🧰 ${tool.name} можно купить в магазине инструментов`);
        }
        // Не оставляем игрока искать магазин самому: сразу открываем его.
        openTools();
        return;
    }
...
function performRepair() {
    ...
    const order = state.currentOrder;
    const repair = order && order.repair;

    // Первый полноценный интерактивный ремонт: замена тормозных колодок.
    // Остальные виды ремонта пока проходят по старой логике, чтобы не ломать игру.
    if (repair && (order.selectedCause === "pads" || /тормозн.*колод/i.test(repair.name || ""))) {
        startBrakePadAnimation();
        return;
    }
...
function startBrakePadAnimation() {
    const order = state.currentOrder;
    if (!order || !order.repair) return;

    openModal(`
        <div class="repair-animation-screen">
            <div class="repair-animation-head">
                <span class="repair-animation-badge">🔧 РЕМОНТ</span>
                <h2>Замена тормозных колодок</h2>
                <p>Сними колесо, извлеки старые колодки и установи новые.</p>
            </div>

            <div class="brake-animation" id="brakeAnimation">
                <div class="brake-wheel">
                    <div class="brake-disc"></div>
                    <div class="brake-caliper"></div>
                    <div class="brake-pad old pad-left"></div>
                    <div class="brake-pad old pad-right"></div>
                    <div class="brake-hub"></div>
                </div>
                <div class="repair-tool">🔧</div>
                <div class="repair-hand">🖐️</div>
            </div>

            <div class="repair-progress-wrap">
                <div class="repair-progress"><div id="repairAnimationProgress"></div></div>
                <div class="repair-step" id="repairAnimationStep">Готов к работе</div>
            </div>

            <button class="action-button" id="startBrakeRepairBtn" onclick="runBrakePadAnimation()">🔧 Начать ремонт</button>
        </div>
    `);
}

function runBrakePadAnimation() {
    const btn = document.getElementById("startBrakeRepairBtn");
    const scene = document.getElementById("brakeAnimation");
    const progress = document.getElementById("repairAnimationProgress");
    const step = document.getElementById("repairAnimationStep");
    if (!btn || !scene || !progress || !step) return;

    btn.disabled = true;
    btn.textContent = "⏳ Работаем...";

    const steps = [
        ["wheel", "Снимаем колесо..."],
        ["old", "Извлекаем старые колодки..."],
        ["clean", "Очищаем тормозной механизм..."],
        ["new", "Устанавливаем новые колодки..."],
        ["finish", "Проверяем свободный ход..." ]
    ];

    scene.classList.add("repair-running");
    let i = 0;

    const next = () => {
        if (i >= steps.length) {
            progress.style.width = "100%";
            step.textContent = "✅ Замена завершена";
            btn.disabled = false;
            btn.textContent = "🧪 Завершить и проверить";
            btn.onclick = performRepairCore;
            scene.classList.remove("repair-running");
            scene.classList.add("repair-complete");
            return;
        }

        const [cls, text] = steps[i];
        step.textContent = text;
        scene.classList.remove("step-wheel", "step-old", "step-clean", "step-new", "step-finish");
        scene.classList.add("step-" + cls);
...
function performRepairCore() {
    ...
    openModal(`
        ...
        <div class="stat-row">
            <span>${repair.consumable ? "Расходный материал" : repair.serviceOnly ? "Услуга" : "Запчасть"}</span>
...
const trainingTests = {
    ...
    engine: {
        ...
        questions: [
            ...
            { q:"После прогрева появился металлический цокот. Какой первый шаг наиболее разумен до разбора двигателя?", a:["Зафиксировать условия появления шума и проверить базовые параметры двигателя","Сразу разбирать клапанный механизм","Сразу менять цепь или ремень ГРМ","Сразу менять все гидрокомпенсаторы"], correct:0 },
            { q:"После ремонта ГРМ появилась ошибка синхронизации. Что проверить первым?", a:["Фактическое совпадение меток и правильность установки ГРМ","Сразу заменить датчик положения распредвала","Сразу заменить датчик положения коленвала","Сразу заменить блок управления двигателем"], correct:0 }
        ]
    },
    electrical: {
        ...
        questions: [
            ...
            { q:"Стартер крутит медленно, аккумулятор заряжен. Что следует проверить первым из перечисленного?", a:["Падение напряжения в силовой цепи стартера и массе во время прокрутки","Давление топлива в рампе","Температуру охлаждающей жидкости","Толщину тормозных дисков"], correct:0 },
            { q:"После установки дополнительного оборудования аккумулятор разряжается за ночь. Что проверить первым?", a:["Ток покоя автомобиля после перехода блоков в спящий режим","Компрессию двигателя","Давление топлива","Толщину тормозных колодок"], correct:0 }
        ]
    },
    diagnostics: {
        ...
        questions: [
            ...
            { q:"Появился Pending-код, но Check Engine ещё не загорелся. Что правильнее сделать сначала?", a:["Сохранить код и условия появления, затем подтвердить неисправность диагностикой","Сразу заменить деталь, связанную с кодом","Сразу стереть код и считать проблему решённой","Сразу заменить аккумулятор"], correct:0 }
        ]
    },
    fuel: {
        ...
        questions: [
            ...
            { q:"Коррекция смеси заметно выше на холостом ходу, чем под нагрузкой. Какая причина особенно характерна для такого поведения?", a:["Подсос воздуха во впускном тракте после измерения воздуха","Износ тормозных колодок","Низкий уровень охлаждающей жидкости","Проблема с подшипником ступицы"], correct:0 }
        ]
    },
    suspension: {
        ...
        questions: [
            ...
            { q:"После регулировки углов автомобиль всё ещё тянет. Какой узел особенно важно исключить, если увод появился именно при торможении?", a:["Подклинивание тормозного механизма","Салонный фильтр","Свечи зажигания","Уровень топлива"], correct:0 }
        ]
    },
    brakes: {
        ...
        questions: [
            ...
            { q:"Одно колесо заметно горячее после короткой поездки. Какая причина наиболее вероятна?", a:["Подклинивание тормозного механизма или ограничение свободного хода колодки","Недостаток топлива в баке","Низкий уровень масла в двигателе","Неисправность салонного фильтра"], correct:0 }
        ]
    },
    transmission: {
        ...
        questions: [
            ...
            { q:"АКПП работает нормально холодной, но после прогрева появляются задержки. Что сравнить в первую очередь?", a:["Поведение коробки и диагностические параметры при холодной и рабочей температуре","Толщину тормозных дисков","Компрессию двигателя","Давление в шинах"], correct:0 }
        ]
    },
    cooling: {
        ...
        questions: [
            ...
            { q:"Перегрев возникает в пробке, а на трассе температура нормальная. Какой узел логичнее проверить первым?", a:["Работу вентилятора охлаждения и его включение при малой скорости","Свечи зажигания","Тормозные колодки","Салонный фильтр"], correct:0 }
        ]
    },
    engine_types: {
        ...
        questions: [
            ...
            { q:"На V6 пропуск появляется только по одной стороне двигателя. Какой первый диагностический шаг наиболее логичен?", a:["Определить, какие цилиндры относятся к этой банке, и проверить их по отдельности","Сразу заменить обе катушки всех цилиндров","Сразу заменить оба кислородных датчика","Сразу заменить блок управления двигателем"], correct:0 }
        ]
    }
};
...
function carPhotoPath(car) {
    // Фото-референсы с Wikimedia Commons. Используются только реальные
    // фотографии автомобилей; если конкретной модели нет — подбирается
    // близкий кузов, а встроенный fallback остаётся доступным.
    const key = `${car.brand} ${car.model}`.toLowerCase();
    const photos = {
        "kia ceed":"https://commons.wikimedia.org/wiki/Special:Redirect/file/Kia_cee%27d_Front.JPG",
        "volkswagen tiguan":"https://commons.wikimedia.org/wiki/Special:Redirect/file/Volkswagen_Tiguan_front-view.JPG",
        "volkswagen passat":"https://commons.wikimedia.org/wiki/Special:Redirect/file/VW_Passat_front.JPG",
        "audi a4":"https://commons.wikimedia.org/wiki/Special:Redirect/file/Audi_A4.jpg",
        "bmw 3 series":"https://commons.wikimedia.org/wiki/Special:Redirect/file/BMW-3-Series_cars.jpg",
        "mercedes-benz c-class":"https://commons.wikimedia.org/wiki/Special:Redirect/file/05-07_Mercedes-Benz_C-Class.jpg",
        "hyundai solaris":"https://commons.wikimedia.org/wiki/Special:Redirect/file/Hyundai-Accent-hatchback-front-1.jpg",
        "hyundai i30":"https://commons.wikimedia.org/wiki/Special:Redirect/file/Honda_Civic_Hatchback_(front).jpg",
        "honda civic":"https://commons.wikimedia.org/wiki/Special:Redirect/file/Honda_Civic_Hatchback_(front).jpg"
    };
    if (photos[key]) return photos[key];
    if (/tiguan|sportage|tucson|rav4|cr-v|outlander|duster|yeti|xc60|rx/i.test(key)) return photos["volkswagen tiguan"];
    if (/passat|octavia|corolla|elantra|polo|rapid|logan|vesta|granta|priora|sandero|ceed|i30/i.test(key)) return photos["kia ceed"];
    if (/audi/i.test(key)) return photos["audi a4"];
    if (/bmw/i.test(key)) return photos["bmw 3 series"];
    if (/mercedes/i.test(key)) return photos["mercedes-benz c-class"];
    return photos["hyundai solaris"];
}
...
function auctionCarImage(car, cls = "auction-car-photo") {
    ...
    return `<img class="${cls}" src="${car.photo || carPhotoPath(car)}" alt="${safe}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';"><div class="auction-car-fallback">${car.icon || "🚗"}<span>${safe}</span></div>`;
}
...
function garageSceneHtml() {
    ...
    return `
        <div class="garage-scene" id="garageScene">
            ...
            <div class="garage-car real-photo-car interactive" id="visualCar" onclick="inspectCar()" title="Нажми для осмотра">
                <img class="real-car-photo" src="${order ? carPhotoPath(order.car) : carPhotoPath({brand:'Hyundai',model:'Solaris'})}" alt="${carName}" onerror="this.style.display='none';this.parentElement.classList.add('photo-fallback')">
            </div>
...
/* =========================================================
   МОБИЛЬНОЕ ИСПРАВЛЕНИЕ v06.4
   Нижнее меню не перекрывает модальные окна, а длинные окна
   нормально прокручиваются вверх/вниз на телефоне.
========================================================= */
(function injectMobileModalFix(){
    if(document.getElementById("mobileModalNavigationFix")) return;
    const style=document.createElement("style");
    style.id="mobileModalNavigationFix";
    style.textContent=`
        /* Нижняя панель всегда поверх страницы, но НЕ поверх модального окна. */
        .bottom-navigation{
            z-index:450 !important;
            padding-bottom:calc(7px + env(safe-area-inset-bottom)) !important;
        }

        /* Когда открыто модальное окно, нижняя навигация скрыта.
           Поэтому последние строки контента больше не оказываются под меню. */
        body.modal-open #bottomNavigation{display:none !important;}

        /* Надёжная прокрутка длинных модальных окон на Android/iOS. */
        #modal{
            touch-action:pan-y;
        ...
        #modal.auction-modal .modal-window{
...
/* =========================================================
   FINAL EDITION v1.0 — КАРЬЕРА, КОМАНДА, ЭНДГЕЙМ
   Добавлено поверх рабочей версии без удаления существующих систем.
========================================================= */
(function finalEdition(){
    if (window.__autoServiceFinalEdition) return;
    window.__autoServiceFinalEdition = true;
    ...
    state.business = Object.assign({
        ...
        lastBusinessDay: Number(state.day || 1)
    }, state.business || {});
    state.business.employees = Object.assign({}, state.business.employees || {});
    state.business.milestones = Object.assign({}, state.business.milestones || {});

    const employees = {
        apprentice: {icon:'👨‍🔧', name:'Ученик механика', cost:18000, salary:900, level:2, bonus:900, rep:1, desc:'Помогает с простыми работами. Небольшой ежедневный доход сервиса.'},
        mechanic: {icon:'🔧', name:'Механик', cost:45000, salary:1800, level:4, bonus:2200, rep:2, desc:'Берёт на себя часть стандартных заказов и повышает пропускную способность.'},
        diagnostician: {icon:'💻', name:'Диагност', cost:90000, salary:3200, level:6, bonus:4200, rep:3, desc:'Повышает доход от сложной диагностики и помогает сервису получать хорошие отзывы.'},
        master: {icon:'🏆', name:'Мастер-приёмщик', cost:160000, salary:5000, level:8, bonus:7000, rep:4, desc:'Привлекает более дорогих клиентов и увеличивает дневной оборот.'},
        manager: {icon:'📋', name:'Управляющий', cost:300000, salary:7500, level:10, bonus:11000, rep:5, desc:'Открывает полноценный режим управления сервисом и максимальный дневной бонус.'}
    };

    const garageLevels = [
        {level:2,name:'Второй пост',cost:25000,rep:10,posts:2,desc:'Можно обслуживать больше автомобилей.'},
        {level:3,name:'Большой гараж',cost:60000,rep:20,posts:2,desc:'Открывает тяжёлые диагностические работы.'},
        {level:4,name:'Диагностический пост',cost:120000,rep:30,posts:3,desc:'Серьёзное диагностическое оборудование и больше клиентов.'},
        {level:5,name:'Полноценная СТО',cost:250000,rep:40,posts:3,desc:'Можно начинать собирать команду.'},
        {level:6,name:'СТО + шиномонтаж',cost:400000,rep:50,posts:4,desc:'Дополнительный поток заказов и колёсные работы.'},
        {level:7,name:'Кузовной пост',cost:650000,rep:60,posts:4,desc:'Сервис превращается в мультибрендовую мастерскую.'},
        {level:8,name:'Премиум-зона',cost:950000,rep:70,posts:5,desc:'Дорогие автомобили и высокомаржинальные работы.'},
        {level:9,name:'Автотехцентр',cost:1400000,rep:82,posts:6,desc:'Большой техцентр с несколькими направлениями.'},
        {level:10,name:'Мастерская мечты',cost:2200000,rep:95,posts:7,desc:'Финальная ступень карьеры. Ты построил собственный автотехцентр.'}
    ];

    const achievements = [
        {id:'first_order',icon:'🔧',name:'Первый заказ',desc:'Заверши первый ремонт.',ok:()=>state.history.length>=1},
        {id:'ten_orders',icon:'🔟',name:'Опытный мастер',desc:'Заверши 10 заказов.',ok:()=>state.history.length>=10},
        {id:'hundred_orders',icon:'💯',name:'Сотня машин',desc:'Заверши 100 заказов.',ok:()=>state.history.length>=100},
        {id:'rich',icon:'💰',name:'Первые большие деньги',desc:'Накопи 100 000 ₽.',ok:()=>state.money>=100000},
        {id:'million',icon:'💎',name:'Миллионер сервиса',desc:'Накопи 1 000 000 ₽.',ok:()=>state.money>=1000000},
        {id:'tools',icon:'🧰',name:'Оснащённая мастерская',desc:'Купи 5 инструментов.',ok:()=>Object.values(state.tools||{}).filter(Boolean).length>=6},
        {id:'warehouse',icon:'📦',name:'Вторая жизнь деталей',desc:'Восстанови 5 деталей.',ok:()=>((state.partsWarehouse||[]).filter(x=>x.status==='restored').length)>=5},
        {id:'fleet',icon:'🚗',name:'Автопарк',desc:'Имей 3 собственных автомобиля.',ok:()=>((state.ownedCars||[]).length)>=3},
        {id:'qualifications',icon:'🎓',name:'Универсальный мастер',desc:'Получи 8 допусков.',ok:()=>qualificationCount()>=8},
        {id:'dream',icon:'🏆',name:'Мастерская мечты',desc:'Достигни 10 уровня СТО и 95 репутации.',ok:()=>state.garage.level>=10&&state.reputation>=95}
    ];

    function ensureBusiness(){
        state.business = state.business || {};
        state.business.employees = state.business.employees || {};
        state.business.milestones = state.business.milestones || {};
    }

    function completedOrders(){ return Array.isArray(state.history) ? state.history.length : 0; }

    function businessScore(){
        return Math.min(100, Math.round(
            state.reputation * .45 +
            state.level * 2 +
            state.garage.level * 3 +
            Object.values(state.business.employees||{}).filter(Boolean).length * 3
        ));
    }
    ...
    function checkAchievements(){
        ...
        achievements.forEach(a=>{
            if (!state.business.milestones[a.id] && a.ok()) {
                state.business.milestones[a.id] = state.day;
                showToast(`${a.icon} Достижение: ${a.name}`);
            }
        });
    }

    window.hireFinalEmployee = function(id){
        ensureBusiness();
        const e=employees[id];
        if(!e) return;
        if(state.business.employees[id]){showToast('👥 Этот сотрудник уже работает у тебя');return;}
        if(state.level<e.level){showToast(`🔒 Нужен уровень мастерства ${e.level}`);return;}
        if(state.garage.level<Math.max(2, Math.ceil(e.level/2))){showToast('🏢 Сначала развей сервис');return;}
        if(state.money<e.cost){showToast('💰 Недостаточно денег на найм');return;}
        state.money-=e.cost;
        state.business.employees[id]=true;
        addReputation(e.rep);
        save(); render(); checkAchievements();
        showToast(`${e.icon} ${e.name} принят на работу`);
        openServiceManagement();
    };

    window.upgradeGarageFinal = function(){
        const next=garageLevels.find(x=>x.level===state.garage.level+1);
        if(!next){showToast('🏆 Сервис уже достиг максимального уровня');return;}
        if(state.reputation<next.rep){showToast(`⭐ Нужно ${next.rep} репутации`);return;}
        if(state.money<next.cost){showToast(`💰 Нужно ${money(next.cost)}`);return;}
        state.money-=next.cost;
        state.garage.level=next.level;
        state.garage.posts=next.posts;
        state.equipment=Math.min(100,state.equipment+20);
        addXP(75);
        save(); render(); checkAchievements();
        showToast(`🏢 ${next.name} открыт!`);
        openServiceManagement();
    };

    window.claimDailyBusinessBonus = function(){
        ensureBusiness();
        if(state.business.lastBusinessDay===state.day && state.business.dailyBonusClaimed){showToast('📋 Сегодняшний бонус уже получен');return;}
        const workers=Object.entries(state.business.employees).filter(([,v])=>v).map(([id])=>employees[id]);
        const income=2500 + workers.reduce((s,e)=>s+e.bonus,0) + Math.max(0,state.garage.level-1)*500;
        state.money+=income;
        state.business.totalRevenue+=income;
        state.business.bestDayRevenue=Math.max(state.business.bestDayRevenue,income);
        state.business.dailyBonusClaimed=true;
        state.business.lastBusinessDay=state.day;
        addXP(20);
        save(); render();
        showToast(`📈 Дневной оборот +${money(income)}`);
        openServiceManagement();
    };

    window.openServiceManagement = function(){
        ensureBusiness();
        const next=garageLevels.find(x=>x.level===state.garage.level+1);
        const workers=Object.entries(state.business.employees).filter(([,v])=>v).map(([id])=>employees[id]);
        const salary=workers.reduce((s,e)=>s+e.salary,0);
        const achievementsHtml=achievements.map(a=>`<div class="achievement ${state.business.milestones[a.id]?'':'locked'}"><span class="achievement-icon">${a.icon}</span><div><b>${a.name}</b><div class="final-muted">${a.desc}</div></div></div>`).join('');
        const employeeHtml=Object.entries(employees).map(([id,e])=>{
            const owned=!!state.business.employees[id];
            const unlocked=state.level>=e.level && state.garage.level>=Math.max(2,Math.ceil(e.level/2));
            return `<div class="employee-card ${owned?'owned':''}">
                <div class="employee-top"><h3>${e.icon} ${e.name}</h3><span>ур. ${e.level}</span></div>
                <p>${e.desc}</p>
                <div class="stat-row"><span>${owned?'✅ В штате':'💰 Найм'}</span><b>${owned?money(e.salary)+'/день':money(e.cost)}</b></div>
                ${owned?'<div class="tool-owned">Даёт ежедневный бонус к обороту</div>':unlocked?`<button class="action-button" onclick="hireFinalEmployee('${id}')">👥 Нанять</button>`:`<div class="tool-locked">🔒 Нужен уровень мастера ${e.level} и развитие СТО</div>`}
            </div>`;
        }).join('');
        const done=completedOrders();
        const score=businessScore();
        openModal(`<h2>📊 Управление сервисом</h2>
            <div class="final-dashboard">
                <div class="final-stat"><small>🏢 СТО</small><b>ур. ${state.garage.level}/10</b></div>
                <div class="final-stat"><small>📈 Рейтинг бизнеса</small><b>${score}/100</b></div>
                <div class="final-stat"><small>🔧 Заказов</small><b>${done}</b></div>
                <div class="final-stat"><small>👥 Команда</small><b>${workers.length}/5</b></div>
            </div>
            <div class="career-progress"><div style="width:${score}%"></div></div>
            ${next?`<div class="final-card"><h3>🏗️ Следующий уровень</h3><p class="final-muted">${next.name}<br>${next.desc}</p><div class="stat-row"><span>Стоимость</span><b>${money(next.cost)}</b></div><div class="stat-row"><span>Репутация</span><b>${state.reputation}/${next.rep}</b></div><button class="action-button" ${state.money<next.cost||state.reputation<next.rep?'disabled':''} onclick="upgradeGarageFinal()">🏗️ Развить СТО</button></div>`:`<div class="final-card"><h3>🏆 Мастерская мечты</h3><p class="final-muted">Финальный уровень достигнут. Теперь твоя задача — развивать автопарк, команду, склад и репутацию.</p></div>`}
            <button class="action-button" onclick="claimDailyBusinessBonus()">📈 Получить дневной оборот</button>
            <div class="final-card"><h3>👥 Команда</h3><p class="final-muted">Фонд зарплаты: ${money(salary)} в день.</p><div class="employee-grid">${employeeHtml}</div></div>
            <div class="final-card"><h3>🏆 Достижения</h3>${achievementsHtml}</div>
            <button class="action-button secondary-button" onclick="openService()">🏢 Классическое развитие СТО</button>
            <button class="modal-back-button" onclick="closeModal()">← Назад</button>`);
    };

    window.openCareer = window.openServiceManagement;

    // Расширяем экран собственных автомобилей: обслуживание + лёгкий тюнинг.
    window.tuneOwnedCar = function(index){
        const car=state.ownedCars?.[index]; if(!car)return;
        car.tuning=car.tuning||{engine:0,suspension:0,appearance:0};
        const options=[
            {key:'engine',icon:'⚙️',name:'Технический тюнинг',cost:25000+car.tuning.engine*20000,bonus:5},
            {key:'suspension',icon:'🛞',name:'Подвеска',cost:18000+car.tuning.suspension*14000,bonus:3},
            {key:'appearance',icon:'🎨',name:'Внешний тюнинг',cost:15000+car.tuning.appearance*12000,bonus:0}
        ];
        openModal(`<h2>🚗 Тюнинг — ${car.brand} ${car.model}</h2><div class="diagnosis-result">Уровни: двигатель ${car.tuning.engine}/3 · подвеска ${car.tuning.suspension}/3 · внешний вид ${car.tuning.appearance}/3.</div>${options.map(o=>`<div class="final-card"><h3>${o.icon} ${o.name}</h3><div class="stat-row"><span>Уровень</span><b>${car.tuning[o.key]}/3</b></div><div class="stat-row"><span>Стоимость</span><b>${car.tuning[o.key]>=3?'MAX':money(o.cost)}</b></div><button class="action-button" ${car.tuning[o.key]>=3||state.money<o.cost?'disabled':''} onclick="buyCarTune(${index},'${o.key}',${o.cost},${o.bonus})">${car.tuning[o.key]>=3?'Максимум':'🔧 Установить'}</button></div>`).join('')}<button class="modal-back-button" onclick="openMyCars()">← К автомобилям</button>`);
    };

    window.buyCarTune = function(index,key,cost,bonus){
        const car=state.ownedCars?.[index]; if(!car)return;
        car.tuning=car.tuning||{engine:0,suspension:0,appearance:0};
        if(car.tuning[key]>=3||state.money<cost)return;
        state.money-=cost; car.tuning[key]++;
        if(bonus) car.condition=Math.min(100,(car.condition||50)+bonus);
        state.business.carsFlipped=Number(state.business.carsFlipped||0);
        addXP(20); save(); render(); showToast('🔧 Тюнинг установлен'); tuneOwnedCar(index);
    };

    // Заменяем экран управления автопарком на более полный вариант.
    const oldOpenMyCars = window.openMyCars;
    window.openMyCars = function(){
        const list=state.ownedCars||[];
        openModal(`<h2>🚗 Мои автомобили</h2>${list.length?`<div class="auction-grid">${list.map((car,i)=>`<div class="auction-card"><div class="auction-photo-wrap">${typeof auctionCarImage==='function'?auctionCarImage(car):car.icon||'🚗'}</div><h3>${car.brand} ${car.model}</h3><div class="auction-meta">${car.year} г. · ${Number(car.mileage).toLocaleString('ru-RU')} км</div><div class="auction-meta">Покупка: ${money(car.purchasePrice)}</div><div class="auction-meta">Ремонт: ${money(car.repairCost||0)}</div><div class="auction-meta">Состояние: ${Math.round(car.condition||50)}%</div><button class="action-button" onclick="repairAuctionCar(${i})">🔧 Ремонтировать</button><button class="action-button" onclick="tuneOwnedCar(${i})">⚙️ Тюнинг</button><button class="action-button" onclick="sellOwnedCar(${i})">💰 Продать</button></div>`).join('')}</div>`:'<div class="diagnosis-result">Пока нет купленных автомобилей. Загляни на аукцион.</div>'}<button class="modal-back-button" onclick="closeModal()">← Назад</button>`);
    };

    // Обёртка вокруг нового заказа: начисляем зарплаты и бизнес-бонус при переходе дня.
    const oldNewOrder = window.newOrder;
    window.newOrder = function(){
        ...
        if(state.day!==oldDay){
            const workers=Object.entries(state.business.employees).filter(([,v])=>v).map(([id])=>employees[id]);
            const salary=workers.reduce((s,e)=>s+e.salary,0);
            if(salary){ state.money=Math.max(0,state.money-salary); }
            state.business.dailyBonusClaimed=false;
            state.business.lastBusinessDay=state.day;
            const bonus=workers.reduce((s,e)=>s+e.bonus,0)+Math.max(0,state.garage.level-1)*500;
            if(bonus){ state.money+=bonus; state.business.totalRevenue+=bonus; state.business.bestDayRevenue=Math.max(state.business.bestDayRevenue,bonus); }
            save(); render();
            if(bonus) showToast(`👥 Команда принесла ${money(bonus)} оборота`);
        }
        checkAchievements();
    };

    // После каждого обычного рендера добавляем кнопку управления, не ломая старую сцену.
    const oldRenderGarageScene = window.renderGarageScene;
    window.renderGarageScene = function(){
        oldRenderGarageScene();
        const nav=document.querySelector('.garage-navigation');
        if(nav && !nav.querySelector('.final-management-card')){
            const b=document.createElement('button');
            b.className='garage-nav-card accent final-management-card';
            b.innerHTML='<span class="garage-nav-icon">📊</span><span><b>Управление СТО</b><small>Команда, уровни, достижения</small></span>';
            b.onclick=()=>openServiceManagement();
            nav.appendChild(b);
        }
    };
    ...
    checkAchievements();
    save();
    // Перерисовываем после подключения финального слоя.
