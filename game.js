...
const defaultState = {
    ...
    // Отдельное оборудование для восстановления складских деталей.
    // Каждая установка покупается независимо и имеет собственный ресурс.
    restoreEquipment: {
        washer: { owned: false, resource: 100 },
        ultrasonic: { owned: false, resource: 100 },
        press: { owned: false, resource: 100 },
        rebuildBench: { owned: false, resource: 100 },
        testBench: { owned: false, resource: 100 }
    },
...
// Миграция сохранений: новые параметры оборудования добавляются без сброса прогресса.
state.restoreEquipment = Object.assign(
    {},
    structuredClone(defaultState.restoreEquipment),
    state.restoreEquipment || {}
);
for (const key of Object.keys(defaultState.restoreEquipment)) {
    const item = state.restoreEquipment[key];
    if (typeof item === "boolean") {
        state.restoreEquipment[key] = { owned: item, resource: 100 };
    } else {
        state.restoreEquipment[key] = Object.assign(
            {},
            defaultState.restoreEquipment[key],
            item || {}
        );
        state.restoreEquipment[key].owned = !!state.restoreEquipment[key].owned;
        state.restoreEquipment[key].resource = Math.max(0, Math.min(100, Number(state.restoreEquipment[key].resource) || 0));
    }
}
...
function generateOrder() {
    ...
    do {
        problem = random(pool.length ? pool : problems);
        attempts++;
...
function getRestorePartMeta(item) {
    const text = String(item.restoreType || item.repairName || item.name || "").toLowerCase();

    // Расходники и детали, которые экономически/технически не имеет смысла восстанавливать.
    const noRestore = /(аккумулятор|свеч|колодк|ремень|фильтр|шина|патруб|термостат|прокладк|сальник|топливный насос|датчик|масса|развал|балансировк|проверка|устранение|герметизац|давления)/i.test(text);
    if (noRestore) return { available: false, type: null, title: "Восстановление не предусмотрено" };

    if (/(форсун|соленоид)/i.test(text)) {
        return { available: true, type: "ultrasonic", title: "Очистка и восстановление прецизионного узла", required: ["washer", "ultrasonic", "testBench"], restoreGain: 38, baseMultiplier: 0.82, cost: 1800, time: 90 };
    }
    if (/(подшипник|ступиц|шаровой|суппорт|тормоз)/i.test(text)) {
        return { available: true, type: "press", title: "Восстановление механического узла", required: ["washer", "press", "testBench"], restoreGain: 42, baseMultiplier: 0.78, cost: 2200, time: 100 };
    }
    if (/(генератор|стартер|компрессор кондиционера|турбокомпрессор|турбин|рулевая рейка|соленоид|опора|сцепления|сцепление|акпп|коробк|блок управления|гбц|двигател)/i.test(text)) {
        return { available: true, type: "rebuildBench", title: "Разборка и восстановление агрегата", required: ["washer", "rebuildBench", "testBench"], restoreGain: 48, baseMultiplier: 0.86, cost: 4200, time: 180 };
    }

    // Простые механические детали: достаточно мойки и финальной проверки.
    if (/(диск|радиатор|помп|агрегат|ремонт)/i.test(text)) {
        return { available: true, type: "washer", title: "Очистка и восстановление детали", required: ["washer", "testBench"], restoreGain: 28, baseMultiplier: 0.68, cost: 1200, time: 60 };
    }

    return { available: false, type: null, title: "Восстановление не предусмотрено" };
}

const restoreEquipmentCatalog = [
    { id: "washer", icon: "🧼", name: "Моечная установка", cost: 25000, level: 1, desc: "Очистка старых деталей перед восстановлением." },
    { id: "ultrasonic", icon: "🔊", name: "Ультразвуковая ванна", cost: 40000, level: 2, desc: "Очистка форсунок, соленоидов и мелких прецизионных деталей." },
    { id: "press", icon: "🗜️", name: "Гидравлический пресс", cost: 50000, level: 2, desc: "Работа с подшипниками, ступицами и механическими узлами." },
    { id: "rebuildBench", icon: "🔧", name: "Стенд восстановления агрегатов", cost: 75000, level: 3, desc: "Разборка, ремонт и сборка сложных агрегатов." },
    { id: "testBench", icon: "🧪", name: "Стенд проверки агрегатов", cost: 90000, level: 4, desc: "Финальная проверка восстановленной детали перед продажей." }
];

function restoreEquipmentById(id) {
    return restoreEquipmentCatalog.find(x => x.id === id);
}
function hasRestoreEquipment(id) {
    return !!state.restoreEquipment?.[id]?.owned;
}
function restoreEquipmentResource(id) {
    return Math.max(0, Math.min(100, Number(state.restoreEquipment?.[id]?.resource) || 0));
}
function buyRestoreEquipment(id) {
    const item = restoreEquipmentById(id);
    if (!item) return;
    if (hasRestoreEquipment(id)) { showToast("🧰 Это оборудование уже куплено"); return; }
    if (state.level < item.level) { showToast(`🔒 Нужен уровень мастерства ${item.level}`); return; }
    if (state.money < item.cost) { showToast("💰 Недостаточно денег"); return; }
    state.money -= item.cost;
    state.restoreEquipment[id] = { owned: true, resource: 100 };
    save(); render();
    showToast(`🧰 ${item.name} куплено`);
    openRestoreEquipment();
}
function consumeRestoreEquipment(ids, amount) {
    ids.forEach(id => {
        if (!state.restoreEquipment?.[id]) return;
        state.restoreEquipment[id].resource = Math.max(0, restoreEquipmentResource(id) - amount);
    });
}
function restoreEquipmentMissing(ids) {
    return ids.filter(id => !hasRestoreEquipment(id) || restoreEquipmentResource(id) <= 0);
}

function openRestoreEquipment() {
    const cards = restoreEquipmentCatalog.map(item => {
        const owned = hasRestoreEquipment(item.id);
        const unlocked = state.level >= item.level;
        const resource = restoreEquipmentResource(item.id);
        return `<div class="tool-card ${owned ? 'owned' : ''}">
            <div class="tool-card-top"><div><h3>${item.icon} ${item.name}</h3><p>${item.desc}</p></div><span class="tool-level">Ур. ${item.level}</span></div>
            <div class="stat-row"><span>${owned ? '✅ Куплено' : unlocked ? '🔓 Доступно' : '🔒 Заблокировано'}</span><b>${owned ? `Ресурс ${resource}%` : money(item.cost)}</b></div>
            ${owned ? `<div class="tool-owned">Можно использовать при восстановлении деталей</div>` : unlocked ? `<button class="action-button" onclick="buyRestoreEquipment('${item.id}')">🛒 Купить за ${money(item.cost)}</button>` : `<div class="tool-locked">Требуется уровень мастерства ${item.level}</div>`}
        </div>`;
    }).join('');

    openModal(`<h2>🛠️ Оборудование для восстановления</h2>
        <div class="diagnosis-result">Каждая установка покупается отдельно. Оборудование расходует собственный ресурс при восстановлении деталей.</div>
        <div class="stat-row"><span>🔧 Уровень мастерства</span><b>${state.level}</b></div>
        <div class="stat-row"><span>💰 Деньги</span><b>${money(state.money)}</b></div>
        <div class="tool-grid">${cards}</div>
        <button class="modal-back-button" onclick="openGarageParts()">← Вернуться на склад</button>`);
}

function sellWarehousePart(index) {
    const items = state.partsWarehouse || [];
    const item = items[index];
    if (!item) return;
    const condition = Math.max(1, Number(item.condition) || 1);
    const baseCost = Math.max(300, Number(item.partCost) || 300);
    const price = Math.max(300, Math.round(baseCost * (0.18 + condition / 100 * 0.42)));
    state.money += price;
    items.splice(index, 1);
    save(); render();
    openModal(`<h2>💰 Деталь продана</h2><div class="diagnosis-result"><b>${item.name}</b><br>Состояние: ${condition}%<br><br>💰 Получено: <b>+${money(price)}</b><br><small>Продажа без восстановления — быстро, но дешевле восстановленной детали.</small></div><button class="action-button" onclick="openGarageParts()">← Вернуться на склад</button>`);
}

function restoreWarehousePart(index) {
    const items = state.partsWarehouse || [];
    const item = items[index];
    if (!item) return;
    if (item.restored || item.status === "restored") {
        openModal(`<h2>♻️ Деталь уже восстановлена</h2><div class="diagnosis-result"><b>${item.name}</b><br>Состояние: <b>${item.condition}%</b><br><br>Повторное восстановление этой детали не требуется. Её можно продать со склада.</div><button class="action-button" onclick="openGarageParts()">📦 Вернуться на склад</button>`);
        return;
    }
    const meta = getRestorePartMeta(item);
    if (!meta.available) {
        openModal(`<h2>🔒 Восстановление недоступно</h2><div class="diagnosis-result">Для детали <b>${item.name}</b> восстановление в текущей версии не предусмотрено.<br><br>💰 Её можно продать как есть.</div><button class="action-button" onclick="openGarageParts()">← Вернуться на склад</button>`);
        return;
    }
    const missing = restoreEquipmentMissing(meta.required);
    if (missing.length) {
        const names = missing.map(id => { const e=restoreEquipmentById(id); return `${e?.icon||'🧰'} ${e?.name||id}`; }).join('<br>');
        openModal(`<h2>🛠️ Не хватает оборудования</h2><div class="diagnosis-result"><b>${item.name}</b><br><br>Для восстановления нужно:<br>${meta.required.map(id=>{const e=restoreEquipmentById(id);return `${hasRestoreEquipment(id)&&restoreEquipmentResource(id)>0?'✅':'❌'} ${e.icon} ${e.name} — ${hasRestoreEquipment(id)?`ресурс ${restoreEquipmentResource(id)}%`:`не куплено`}`;}).join('<br>')}</div><button class="action-button" onclick="openRestoreEquipment()">🛒 Открыть оборудование</button><button class="modal-back-button" onclick="openGarageParts()">← Назад</button>`);
        return;
    }
    openModal(`<h2>♻️ Восстановление детали</h2>
        <div class="diagnosis-result"><b>${item.name}</b><br>Состояние сейчас: <b>${item.condition}%</b><br><br>🔧 ${meta.title}<br>⏱️ Время: ${meta.time} мин<br>💸 Расход на восстановление: ${money(meta.cost)}<br>📈 После работы: до ${Math.min(100, Number(item.condition)+meta.restoreGain)}%</div>
        <button class="action-button" onclick="confirmRestoreWarehousePart(${index})">♻️ Восстановить</button>
        <button class="modal-back-button" onclick="openGarageParts()">← Назад</button>`);
}

function confirmRestoreWarehousePart(index) {
    const items = state.partsWarehouse || [];
    const item = items[index];
    if (!item) return;
    const meta = getRestorePartMeta(item);
    if (!meta.available) { showToast("🔒 Эту деталь восстановить нельзя"); return; }
    const missing = restoreEquipmentMissing(meta.required);
    if (missing.length) { showToast("🛠️ Не хватает оборудования"); return; }
    if (state.money < meta.cost) { showToast("💰 Недостаточно денег на восстановление"); return; }

    state.money -= meta.cost;
    consumeRestoreEquipment(meta.required, meta.required.includes("rebuildBench") ? 10 : 6);
    item.condition = Math.min(100, Number(item.condition || 0) + meta.restoreGain);
    item.status = "restored";
    item.restored = true;
    item.restoreType = meta.type;
    item.restoreCost = (Number(item.restoreCost)||0) + meta.cost;
    save(); render();

    const salePrice = Math.max(500, Math.round((Number(item.partCost)||1000) * meta.baseMultiplier * (0.55 + item.condition/100*0.45)));
    openModal(`<h2>✅ Деталь восстановлена</h2><div class="diagnosis-result"><b>${item.name}</b><br>Состояние: <b>${item.condition}%</b><br><br>💸 Затрачено: ${money(meta.cost)}<br>💰 Ориентировочная цена продажи: <b>${money(salePrice)}</b></div><button class="action-button" onclick="openGarageParts()">📦 Вернуться на склад</button>`);
}
...
function storeOldPart() {
    ...
    const partName = repair.name || repair.partName || "Старая деталь";
    const item = {
        ...
        status: "used",
        restored: false,
        restoreAvailable: getRestorePartMeta({repairName: repair.name}).available,
        partCost: Number(repair.partCost) || 0,
        partId: repair.partId || null,
        originalPartName: repair.partName || null
    };
    ...
    openModal(`
        ...
        <div class="diagnosis-result">
            ...
            🚗 ${item.car}<br><br>
            ${item.restoreAvailable ? '♻️ Деталь можно будет восстановить при наличии нужного оборудования.' : '💰 Эту деталь можно будет продать как есть.'}
        </div>
        <button class="action-button" onclick="newOrder()">🚗 Следующий клиент</button>
    `);
}
...
/* =========================================================
   НИЖНЯЯ НАВИГАЦИЯ — ПОЛНОЦЕННАЯ ВЕРСИЯ V40
========================================================= */

function injectBottomNavigationStyles() {
    if (document.getElementById("bottomNavigationStyles")) return;

    const style = document.createElement("style");
    style.id = "bottomNavigationStyles";
    style.textContent = `
        body { padding-bottom: 84px !important; }

        /* Модальные окна должны быть выше нижней навигации.
           Иначе меню «Ещё» частично перекрывается панелью. */
        #modal {
        ...
        .bottom-navigation button .nav-icon {
            font-size: 20px;
            line-height: 1;
        }
        ...
        .bottom-navigation-more button:active { transform: scale(.98); }
        @media (max-width: 370px) {
            .bottom-navigation { height: 68px; }
            .bottom-navigation button { font-size: 9px; }
            .bottom-navigation button .nav-icon { font-size: 18px; }
        }
    `;
    document.head.appendChild(style);
}

function scrollToGame(selector) {
...
function setBottomNavActive(id) {
    document.querySelectorAll(".bottom-navigation button[data-nav]").forEach(btn => {
        btn.classList.toggle("active", btn.dataset.nav === id);
    });
}

function openBottomMore() {
    openModal(`
        <h2>☰ Ещё</h2>
        <p>Дополнительные разделы сервиса</p>
        <div class="bottom-navigation-more">
            <button onclick="closeModal(); openGarageParts();">
                📦<br><b>Склад</b>
            </button>
            <button onclick="closeModal(); openGarageTools();">
                🧰<br><b>Оборудование</b>
            </button>
            <button onclick="closeModal(); openTraining();">
                📚<br><b>Обучение</b>
            </button>
            <button onclick="closeModal(); openCars();">
                🚘<br><b>Автомобили</b>
            </button>
            <button onclick="closeModal(); openService();">
                🏢<br><b>Развитие СТО</b>
            </button>
            <button onclick="closeModal(); scrollToGame('.garage-image');">
                🔧<br><b>Гараж</b>
            </button>
        </div>
    `);
...
function renderBottomNavigation() {
    ...
    let nav = document.getElementById("bottomNavigation");
    if (!nav) {
        nav = document.createElement("nav");
        nav.id = "bottomNavigation";
        nav.className = "bottom-navigation";
        nav.innerHTML = `
            <button data-nav="home" onclick="scrollToGame('.app'); setBottomNavActive('home');">
                <span class="nav-icon">🏠</span><span>Главная</span>
            </button>
            <button data-nav="orders" onclick="scrollToGame('.order-card'); setBottomNavActive('orders');">
                <span class="nav-icon">🚗</span><span>Заказы</span>
            </button>
            <button data-nav="garage" onclick="scrollToGame('.garage-image'); setBottomNavActive('garage');">
                <span class="nav-icon">🔧</span><span>Гараж</span>
            </button>
            <button data-nav="knowledge" onclick="openTraining(); setBottomNavActive('knowledge');">
                <span class="nav-icon">📚</span><span>База</span>
            </button>
            <button data-nav="more" onclick="openBottomMore(); setBottomNavActive('more');">
    ...
    return `
        <div class="garage-scene" id="garageScene">
            ...
            <div class="garage-actions">
                <button class="garage-action-button" onclick="openGarageParts()">📦 Склад запчастей <span>${(state.partsWarehouse||[]).length}</span></button>
                <button class="garage-action-button" onclick="openGarageTools()">🧰 Оборудование</button>
            </div>
...
function openGarageParts() {
    ...
    const rows = items.length ? items.map((item, i) => {
        ...
        const condition = Math.max(1, Number(item.condition) || 1);
        const baseCost = Math.max(300, Number(item.partCost) || 300);
        const asIsPrice = Math.max(300, Math.round(baseCost * (0.18 + condition / 100 * 0.42)));
        const restoredPrice = meta.available ? Math.max(500, Math.round(baseCost * meta.baseMultiplier * (0.55 + condition / 100 * 0.45))) : 0;
        return `<div class="diagnosis-result" style="margin-top:10px">
            <b>${i + 1}. ${item.name}</b><br>
            🚗 ${item.car}<br>
            Состояние: <b>${condition}%</b> ${item.restored ? '♻️ Восстановлена' : '🔩 Б/у'}<br>
            <div class="stat-row"><span>💰 Продать как есть</span><b>${money(asIsPrice)}</b></div>
            ${meta.available ? `<div class="stat-row"><span>♻️ После восстановления</span><b>≈ ${money(restoredPrice)}</b></div>
                <button class="action-button" onclick="restoreWarehousePart(${i})">♻️ Восстановить</button>` : `<div class="tool-locked">🔒 Восстановление для этой детали не предусмотрено</div>`}
            <button class="action-button secondary-button" onclick="sellWarehousePart(${i})">💰 Продать как есть</button>
        </div>`;
    }).join('') : `
        <div class="diagnosis-result">Склад пока пуст. Старые детали, которые клиенты оставят после ремонта, будут попадать сюда.</div>
    `;
    ...
    openModal(`
        ...
        ${rows}
        <button class="action-button" onclick="openRestoreEquipment()">🛠️ Оборудование для восстановления</button>
        <button class="modal-back-button" onclick="closeModal()">← Назад</button>
    `);
}
