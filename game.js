...
function isOrderCompatible(order) {
    ...
    // Важная проверка: двигатель и коробка должны существовать
    // именно у выбранной модели автомобиля. Это защищает от старых
    // сохранений вроде "Volkswagen Polo + V8".
    const carDef = cars.find(c =>
        c.brand === order.car.brand &&
        c.model === order.car.model
    );
    if (!carDef) return false;

    if (!carDef.engines.includes(engine)) return false;
    if (!carDef.transmissions.includes(transmission)) return false;
    ...
    openModal(`
        ...
        <div class="car-inspection-card">
            ...
            <div>
                ...
                <p class="inspection-specs">
                    ${order.car.engine} · ${order.car.transmission}
                </p>
            </div>
        </div>
